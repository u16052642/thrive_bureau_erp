...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('thrive.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='thrive')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='thrive.tools.image')
    ...
    # Check that handler.stream has a fileno() method: when running ThriveERP
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("THRIVE_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'thrive.http.rpc.request:INFO',
    'thrive.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['thrive:DEBUG', 'thrive.sql_db:INFO', 'thrive.http.rpc:DEBUG'],
    'debug_rpc': ['thrive:DEBUG', 'thrive.sql_db:INFO', 'thrive.http.rpc.request:DEBUG'],
    'debug': ['thrive:DEBUG', 'thrive.sql_db:INFO'],
    'debug_sql': ['thrive.sql_db:DEBUG'],
    'info': [],
    'runbot': ['thrive:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['thrive:WARNING', 'werkzeug:WARNING'],
    'error': ['thrive:ERROR', 'werkzeug:ERROR'],
    'critical': ['thrive:CRITICAL', 'werkzeug:CRITICAL'],
}
