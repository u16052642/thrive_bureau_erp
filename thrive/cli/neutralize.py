...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = thrive.tools.config.parser
        ...
        opt = thrive.tools.config.parse_config(args)

        dbname = thrive.tools.config['db_name']
        if not dbname:
        ...
        try:
            with thrive.sql_db.db_connect(dbname).cursor() as cursor:
                installed_modules = thrive.modules.neutralize.get_installed_modules(cursor)
                queries = thrive.modules.neutralize.get_neutralization_queries(installed_modules)
                if opt.to_stdout:
