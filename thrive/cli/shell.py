...
class Shell(Command):
    """Start thrive in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        thrive.cli.server.report_configuration()
        thrive.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = thrive.api.Environment(cr, uid, {})['res.users'].context_get()
                env = thrive.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
