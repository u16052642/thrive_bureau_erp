...
from thrive.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    Thrive cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        thrive-bin cloc -p module_path

    Or by providing the name of a database:

        thrive-bin cloc --addons-path=dirs -d database
