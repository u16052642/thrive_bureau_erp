...
def is_initialized(cr):
    ...
    """
    return thrive.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = thrive.modules.get_module_resource('base', 'data', 'base_data.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with thrive.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in thrive.modules.get_modules():
        mod_path = thrive.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = thrive.modules.get_manifest(i)
...
def has_unaccent(cr):
    ...
    module but any similar function will be picked by ThriveERP.
...
def has_trigram(cr):
    ...
    pg_trgm module but any similar function will be picked by Thrive.
