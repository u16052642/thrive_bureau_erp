...
import thrive
from thrive.modules.db import FunctionStatus
from thrive.osv.expression import get_unaccent_wrapper
from .. import SUPERUSER_ID
from thrive.sql_db import TestCursor
from thrive.tools import (config, existing_tables, ignore,
                        lazy_classproperty, lazy_property, sql,
                        Collector, OrderedSet)
from thrive.tools.func import locked
from thrive.tools.lru import LRU

_logger = logging.getLogger(__name__)
_schema = logging.getLogger('thrive.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                thrive.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                thrive.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        self._init = True
        self._assertion_report = thrive.tests.runner.ThriveTestResult()
        ...
        self._db = thrive.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = thrive.modules.db.has_unaccent(cr)
            self.has_trigram = thrive.modules.db.has_trigram(cr)
