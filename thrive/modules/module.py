...
import thrive
import thrive.tools as tools
import thrive.release as release
...
# ad_paths is a deprecated alias, please use thrive.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"thrive.modules.module.ad_paths" is a deprecated proxy to '
        '"thrive.addons.__path__".', DeprecationWarning, stacklevel=2)
    return thrive.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "thrive.addons".',
    ...
    def load_module(self, name):
        ...
        thrive_name = re.sub(r'^openerp.addons.(\w+)$', r'thrive.addons.\g<1>', name)

        thrive_module = sys.modules.get(thrive_name)
        if not thrive_module:
            thrive_module = importlib.import_module(thrive_name)

        sys.modules[name] = thrive_module

        return thrive_module

class ThriveHook(object):
    """ Makes thrive package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to thrive.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'thrive\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `thrive.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^thrive.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("thrive.addons.base.maintenance.migrations", "thrive.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``thrive.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook thrive.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in thrive.addons.__path__:
        thrive.addons.__path__.append(dd)

    # hook thrive.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in thrive.addons.__path__:
            thrive.addons.__path__.append(ad)

    # hook thrive.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in thrive.addons.__path__ and os.path.isdir(base_path):
        thrive.addons.__path__.append(base_path)

    # hook thrive.upgrade on upgrade-path
    from thrive import upgrade
    ...
    # create decrecated module alias from thrive.addons.base.maintenance.migrations to thrive.upgrade
    spec = importlib.machinery.ModuleSpec("thrive.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["thrive.addons.base.maintenance"] = maintenance_pkg
    sys.modules["thrive.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to thrive and "crm"-like to thrive.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, ThriveHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in thrive.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = thrive.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in thrive.addons.__path__:
...
def load_openerp_module(module_name):
    """ Load an ThriveERP module, if not already loaded.
    ...
    try:
        __import__('thrive.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['thrive.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in thrive.addons.__path__:
