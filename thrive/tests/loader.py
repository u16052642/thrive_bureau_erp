...
from .. import tools
from .common import TagsSelector, ThriveSuite
from .runner import ThriveTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'thrive.addons.{module}'))

    upgrade_spec = importlib.util.find_spec(f'thrive.upgrade.{module}')
    if upgrade_spec:
...
def make_suite(module_names, position='at_install'):
    ...
    return ThriveSuite(sorted(tests, key=lambda t: t.test_sequence))
