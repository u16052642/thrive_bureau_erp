...
import thrive
from thrive.tools import config, topological_sort, unique
from thrive.netsvc import init_logger
from thrive.tests import standalone_tests
import thrive.tests.loader

_logger = logging.getLogger('thrive.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with thrive.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of Thrive modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Thrive modules")
...
def test_full(args):
    ...
    with thrive.registry(args.database).cursor() as cr:
        env = thrive.api.Environment(cr, thrive.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = thrive.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        thrive.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        thrive.tools.config['addons_path'] = ','.join([args.addons_path, thrive.tools.config['addons_path']])
        if args.data_dir:
            thrive.tools.config['data_dir'] = args.data_dir
        thrive.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('thrive.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('thrive.sql_db').setLevel(logging.CRITICAL)
