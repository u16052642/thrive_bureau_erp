# -*- coding: utf-8 -*-
"""
The module :mod:`thrive.tests.common` provides unittest test cases and a few
...
from thrive.models import BaseModel
from thrive.exceptions import AccessError
from thrive.modules.registry import Registry
from thrive.osv import expression
from thrive.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from thrive.service import security
from thrive.sql_db import BaseCursor, Cursor
from thrive.tools import float_compare, single_email_re, profiler, lower_logging
from thrive.tools.misc import find_in_path
...
# The thrive library is supposed already configured.
ADDONS_PATH = thrive.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = thrive.SUPERUSER_ID
...
def get_db_name():
    db = thrive.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Thrive module name.
    """
    def register(func):
        # register func by thrive module name
        if func.__module__.startswith('thrive.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most thrive operations;
    """
    if not login:
...
class ThriveSuite(BackportSuite):
    def _handleClassSetUp(self, test, result):
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('thrive.addons.'):
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Thrive-specific code. This class is abstract and
    ...
    def run(self, result):
        ...
        if getattr(testMethod, '_retry', True) and getattr(self, '_retry', True):
            tests_run_count = int(os.environ.get('THRIVE_TEST_FAILURE_RETRIES', 0)) + 1
        else:
            tests_run_count = 1
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~thrive.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        with patch('thrive.models.BaseModel.user_has_groups', user_has_groups):
            yield
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('thrive.sql_db.Cursor.execute', execute):
            with patch('thrive.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/thrive/addons/" in filename:
                        filename = filename.rsplit("/thrive/addons/", 1)[1]
                    if count > expected:
    ...
    def _callSetUp(self):
        ...
        # See https://github.com/thrive/thrive/pull/107572 for more info.
...
class _ErrorCatcher(list):
    ...
    def _complete_traceback(self, initial_tb):
        ...
        # remove traceback root part (thrive_bin, main, loading, ...), as
...
class TransactionCase(BaseCase):
    ...
    def _gc_filestore(cls):
        ...
        with thrive.registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, thrive.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.registry = thrive.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, thrive.SUPERUSER_ID, {})
...
class ChromeBrowser:
    ...
    def __init__(self, test_class):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_thrive')
        self.chrome_pid = None

        otc = thrive.tools.config
...
which leads to stray network requests and inconsistencies."""))
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Thrive widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{thrive.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('thrive.service.http.request.')]
        ...
        if request_threads:
            ...
            thrive.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        thrive.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        if getattr(self, 'session', None):
            thrive.http.root.session_store.delete(self.session)

        self.session = session = thrive.http.root.session_store.new()
        session.update(thrive.http.get_default_session(), db=get_db_name())
        session.context['lang'] = thrive.http.DEFAULT_LANG
        ...
        thrive.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return f"http://{HOST}:{thrive.tools.config['http_port']}"
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "thrive.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "thrive.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('thrive.http.Request._get_profiler_context_manager', route_profiler))
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~thrive.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~thrive.fields.Many2many` fields, the field itself is a
    :class:`~thrive.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~thrive.fields.One2many` are reified as
    :class:`~thrive.tests.common.O2MProxy`.

    Because the :class:`~thrive.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~thrive.tests.common.O2MProxy.new` and
    :meth:`~thrive.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: thrive.models.Model
    ...
    :type view: int | str | thrive.model.Model
    ...
    def _process_fvg(self, model, fvg, level=2):
        ...
        * pre-processed onchanges list
        ...
        eval_context = {
            ...
            "datetime": datetime,
            "context_today": lambda: thrive.fields.Date.context_today(self._env.user),
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~thrive.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~thrive.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = thrive.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = thrive.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return thrive.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return thrive.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from thrive.tests.common have a test_tags
