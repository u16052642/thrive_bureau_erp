...
class Meeting(models.Model):
    ...
    @api.model
    def _thrive_values(self, google_event, default_reminders=()):
        ...
        alarm_commands = self._thrive_reminders_commands(reminder_command)
        attendee_commands, partner_commands = self._thrive_attendee_commands(google_event)
    ...
    @api.model
    def _thrive_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            existing_attendees = self.browse(google_event.thrive_id(self.env)).attendee_ids
        ...
        for thrive_attendee in attendees_by_emails.values():
            ...
            email = tools.email_normalize(thrive_attendee.email)
            if email not in emails and email != self.env.user.email:
                attendee_commands += [(2, thrive_attendee.id)]
                partner_commands += [(3, thrive_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _thrive_reminders_commands(self, reminders=()):
        commands = []
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_thrive_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an Thrive user that do not sync his calendar
            ...
            # the owner is not an Thrive user: We don't store the real owner identity (mail)
            ...
            # values['extendedProperties']['private] should be used if the owner is not an thrive user
