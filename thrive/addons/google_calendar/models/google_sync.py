...
from thrive import api, fields, models, registry, _
from thrive.tools import ormcache_context, email_normalize
from thrive.exceptions import UserError
from thrive.osv import expression

from thrive.addons.google_calendar.utils.google_event import GoogleEvent
from thrive.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from thrive.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Thrive database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Thrive.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Thrive DB (_sync_google2thrive).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_thrive2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2thrive(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in Thrive. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Thrive
        :return: synchronized thrive recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        thrive_values = [
            dict(self._thrive_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_thrive = self.with_context(dont_notify=True)._create_from_google(new, thrive_values)
        cancelled = existing.cancelled()
        cancelled_thrive = self.browse(cancelled.thrive_ids(self.env))
        cancelled_thrive._cancel()
        synced_records = new_thrive + cancelled_thrive
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and thrive server time are different
            updated = parse(gevent.updated)
            thrive_record = self.browse(gevent.thrive_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not thrive_record.write_date or updated >= pytz.utc.localize(thrive_record.write_date):
                vals = dict(self._thrive_values(gevent, default_reminders), need_sync=False)
                thrive_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= thrive_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Thrive Bureau ERP to Google
    ...
    @api.model
    def _thrive_values(self, google_event: GoogleEvent, default_reminders=()):
