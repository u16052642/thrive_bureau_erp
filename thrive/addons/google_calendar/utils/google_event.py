...
from thrive.tools import email_normalize
import logging
...
class GoogleEvent(abc.Set):
    ...
    Inspired by Thrive recordset, one instance can be a single Google event or a
    ...
    def thrive_id(self, env):
        self.thrive_ids(env)  # load ids
        return self._thrive_id

    def _meta_thrive_id(self, dbname):
        """Returns the Thrive id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_thrive_id' % dbname)
        if o_id:
            return int(o_id)

    def thrive_ids(self, env):
        ids = tuple(e._thrive_id for e in self if e._thrive_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_thrive_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_thrive_ids_from_metadata(env, model)

        return tuple(e._thrive_id for e in self)

    def _load_thrive_ids_from_metadata(self, env, model):
        unsure_thrive_ids = tuple(e._meta_thrive_id(env.cr.dbname) for e in self)
        thrive_events = model.browse(_id for _id in unsure_thrive_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Thrive id.
        # Therefore, we only consider Thrive records without google id when trying
        # to match events.
        o_ids = thrive_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            thrive_id = e._meta_thrive_id(env.cr.dbname)
            if thrive_id in o_ids:
                e._events[e.id]['_thrive_id'] = thrive_id

    def _load_thrive_ids_from_db(self, env, model):
        thrive_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in thrive_events}  # {google_id: thrive_id}
        existing_google_ids = thrive_events.mapped('google_id')
        for e in self:
            thrive_id = mapping.get(e.id)
            if thrive_id:
                e._events[e.id]['_thrive_id'] = thrive_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Thrive.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Thrive (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Thrive) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Thrive several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_thrive_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.thrive_ids(env)
        events.thrive_ids(env)

        return self.filter(lambda e: e._thrive_id)
