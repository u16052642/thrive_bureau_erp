...
from thrive.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api
from thrive.addons.google_calendar.utils.google_calendar import GoogleEvent
from thrive.tools import html2plaintext
from thrive import Command

class TestSyncGoogle2Thrive(TestSyncGoogle):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence']._sync_google2thrive(google_recurrence)
        self.env['calendar.event']._sync_google2thrive(events - google_recurrence)
    ...
    def test_new_google_event(self):
        values = {
            ...
            'organizer': {'email': 'thrivecalendarref@gmail.com', 'self': True},
        ...
        self.env['calendar.event']._sync_google2thrive(GoogleEvent([values]))
    ...
    def test_attendee_cancelled(self):
        ...
        self.env['calendar.event'].with_user(user)._sync_google2thrive(gevent)
    ...
    def test_private_extended_properties(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'thrivebot@example.com', 'responseStatus': 'declined'}],
            'extendedProperties': {'private': {'%s_thrive_id' % self.env.cr.dbname: event.id}},
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence']._sync_google2thrive(GoogleEvent([values]))
    ...
    def test_recurrence_moved(self):
        ...
        values = {
            ...
            "attendees": [
                {
                    "email": "thrivebot@example.com", "state": "accepted",
                },
            ],
            'updated': self.now,
        }
    ...
    def test_simple_event_into_recurrency(self):
        ...
        recurrence = self.env['calendar.recurrence']._sync_google2thrive(GoogleEvent([values]))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_attendees_same_event_both_share(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'c.c@example.com', 'responseStatus': 'needsAction'},
                          {'email': 'thrivebot@example.com', 'responseStatus': 'accepted'},],
            'extendedProperties': {'shared': {'%s_thrive_id' % self.env.cr.dbname: event.id,
    ...
    def test_several_attendee_have_the_same_mail(self):
        """
        In google, One mail = One attendee but on Thrive, some partners could share the same mail
        ...
        self.assertEqual(mails, ['dalton@example.com', 'thrivebot@example.com'])
        gevent = GoogleEvent([{
            'id': google_id,
            'description': 'coucou',
            "updated": self.now,
            'organizer': {'email': 'thrivebot@example.com', 'self': True},
            'summary': False,
            'visibility': 'public',
            'attendees': [],
            'reminders': {'useDefault': True},
        ...
        # User attendee removed but gevent owner might be added after synch.
        mails = sorted(set(event.attendee_ids.mapped('email')))
        self.assertEqual(mails, ['thrivebot@example.com'])
    ...
    def test_several_users_have_the_same_mail(self):
        ...
        event = self.env['calendar.event']._sync_google2thrive(GoogleEvent([values]))
    ...
    def test_event_with_meeting_url(self):
        values = {
            ...
            'conferenceData': {
                'entryPoints': [{
                    ...
                    'uri': 'https://meet.google.com/thrive-random-test',
                    'label': 'meet.google.com/thrive-random-test'
                }, {
                    'entryPointType': 'more',
                    'uri':'https://tel.meet/thrive-random-test?pin=42424242424242',
                    'pin':'42424242424242'
                }]
            }
        }
        ...
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
        self.assertTrue(event, "It should have created an event")
        self.assertEqual(event.videocall_location, 'https://meet.google.com/thrive-random-test')
        self.assertGoogleAPINotCalled()
