...
try:
    ...
    from thrive.tools._vendor.send_file import send_file
...
from thrive.modules import get_resource_path
from thrive.tools import file_open, file_path, replace_exceptions
from thrive.tools.mimetypes import guess_mimetype
from thrive.tools.image import image_guess_size_from_field_name


_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Thrive is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Thrive is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if thrive.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=thrive.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        dbname = request.db
        uid = (request.session.uid if dbname else None) or thrive.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                registry = thrive.modules.registry.Registry(dbname)
