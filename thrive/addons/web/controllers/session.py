...
from thrive.modules import module
from thrive.exceptions import AccessError, UserError, AccessDenied
...
class Session(http.Controller):
    ...
    def get_session_info(self):
        # Crapy workaround for unupdatable Thrive Mobile App iOS (Thanks Apple :@)
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        if pre_uid != request.session.uid:
            # Crapy workaround for unupdatable Thrive Mobile App iOS (Thanks Apple :@) and Android
        ...
        registry = thrive.modules.registry.Registry(db)
        with registry.cursor() as cr:
            env = thrive.api.Environment(cr, request.session.uid, request.session.context)
    ...
    def account(self):
        ...
        return 'https://accounts.ThriveERP.com/oauth2/auth?' + url_encode(params)
