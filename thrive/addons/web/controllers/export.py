...
from thrive.tools import lazy_property, osutil, pycompat
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name='',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: thrive.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from Thrive Bureau ERP's export data to whatever the
