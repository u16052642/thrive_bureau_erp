...
from thrive.http import content_disposition, dispatch_rpc, request
from thrive.service import db
from thrive.tools.misc import file_open, str2bool
from thrive.tools.translate import _

from thrive.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = thrive.tools.config.verify_admin_password('admin')
        d['list_db'] = thrive.tools.config['list_db']
        d['langs'] = thrive.service.db.exp_list_lang()
        d['countries'] = thrive.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = thrive.service.db.list_db_incompatible(d['databases'])
        except thrive.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = thrive.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            thrive.service.db.check_super(master_pwd)
            ...
            dump_stream = thrive.service.db.dump_db(name, None, backup_format)
