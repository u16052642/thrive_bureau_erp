    ...
    def test_01_click_everywhere_as_admin(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                ...
                self.browser_js("/web", "thrive.__DEBUG__.services['web.clickEverywhere']('%s');" % menus[app_id]['xmlid'], "thrive.isReady === true", login="admin", timeout=600)
...
class TestMenusDemo(thrive.tests.HttpCase):
    ...
    def test_01_click_everywhere_as_demo(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                _logger.runbot('Testing %s', menus[app_id]['name'])
                self.browser_js("/web", "thrive.__DEBUG__.services['web.clickEverywhere']('%s');" % menus[app_id]['xmlid'], "thrive.isReady === true", login="demo", timeout=600)
                self.terminate_browser()
...
class TestMenusAdminLight(thrive.tests.HttpCase):
    ...
    def test_01_click_apps_menus_as_admin(self):
        self.browser_js("/web", "thrive.__DEBUG__.services['web.clickEverywhere'](undefined, true);", "thrive.isReady === true", login="admin", timeout=120)

@thrive.tests.tagged('post_install', '-at_install',)
class TestMenusDemoLight(thrive.tests.HttpCase):
    allow_end_on_form = True
    def test_01_click_apps_menus_as_demo(self):
        self.browser_js("/web", "thrive.__DEBUG__.services['web.clickEverywhere'](undefined, true);", "thrive.isReady === true", login="demo", timeout=120)
