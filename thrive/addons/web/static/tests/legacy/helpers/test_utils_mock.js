thrive.define('web.test_utils_mock', function (require) {
"use strict";
...
const { Component } = require("@thrive/owl");
...
function _mockGlobalObjects(params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            thrive.debug = params.config.debug;
        }
    }
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any Thrive widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in ThriveAfterTestHook?
 ...
 *   used to generate mock answers for every kind of routes supported by thrive
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught Thrive
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other Thrive events bubbling up are intercepted
