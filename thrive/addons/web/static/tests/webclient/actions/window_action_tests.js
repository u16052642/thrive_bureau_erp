...
import { onMounted } from "@thrive/owl";
let serverData;
let target;
...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("debugManager is active for (legacy) views", async function (assert) {
        ...
        patchWithCleanup(thrive, { debug: "1" });
    ...
    QUnit.test("window action in target new fails (onchange)", async (assert) => {
        ...
        registry
            ...
            .add("thrive.exceptions.ValidationError", WarningDialogWait);

        const mockRPC = (route, args) => {
            if (args.method === "onchange" && args.model === "partner") {
                const error = new RPCError();
                error.exceptionName = "thrive.exceptions.ValidationError";
                error.code = 200;
    ...
    QUnit.test("pushState also changes the title of the tab", async (assert) => {
        ...
        assert.strictEqual(titleService.current, '{"zopenerp":"Thrive","action":"Partners"}');
        await click(target.querySelector(".o_data_row .o_data_cell"));
        assert.strictEqual(titleService.current, '{"zopenerp":"Thrive","action":"First record"}');
        await click(target.querySelector(".o_pager_next"));
        assert.strictEqual(titleService.current, '{"zopenerp":"Thrive","action":"Second record"}');
    });
