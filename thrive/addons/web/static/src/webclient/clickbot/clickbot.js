...
(function (exports) {
    ...
    const BLACKLISTED_MENUS = [
        ...
        "payment_thrive.menu_adyen_account",
        "payment_thrive.root_adyen_menu",
    ];

    const { isEnterprise } = thrive.info;
    const { onWillStart } = owl;
    let appsMenusOnly = false;
    const isStudioInstalled = "@web_studio/studio_service" in thrive.__DEBUG__.services;
    let actionCount = 0;
    ...
    function ensureSetup() {
        ...
        setupDone = true;
        const env = thrive.__WOWL_DEBUG__.root.env;
        env.bus.addEventListener("ACTION_MANAGER:UI-UPDATED", () => {
            actionCount++;
        });

        const AbstractController = thrive.__DEBUG__.services["web.AbstractController"];
        ...
        const { patch } = thrive.__DEBUG__.services["@web/core/utils/patch"];
        const { WithSearch } = thrive.__DEBUG__.services["@web/search/with_search/with_search"];
        ...
        // This test file is not respecting Thrive module dependencies.
        // The following module might not be loaded (eg. if mail is not installed).
        const DiscussWidgetModule = thrive.__DEBUG__.services["@mail/widgets/discuss/discuss"];
    ...
    async function _clickEverywhere(xmlId) {
        ...
        console.log(`Thrive flavor: ${isEnterprise ? "Enterprise" : "Community"}`);
