...
(function () {
    ...
    if (!globalThis.thrive) {
        globalThis.thrive = {};
    }
    var thrive = globalThis.thrive;
    var debug = thrive.debug;
    ...
    thrive.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === "string" ? args.shift() : "__thrive_job" + jobUID++;
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: thrive.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name,
                };
                if (jobs[k].error) {
                ...
                var deps = thrive.__DEBUG__.getDependencies(job.name);
            ...
            missing = thrive.__DEBUG__.getMissingJobs();
            failed = thrive.__DEBUG__.getFailedJobs();
     ...
     */
    thrive.ready = async function (serviceName) {
        function match(name) {
    ...
    thrive.runtimeImport = function (moduleName) {
