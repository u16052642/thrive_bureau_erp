thrive.define('web.data', function (require) {
"use strict";
 ...
 * consumed by ThriveERP's RPC APIs.
 *
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of ThriveERP records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the ThriveERP model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // ThriveERP ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // ThriveERP ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, ThriveERP domain
     ...
     * an SQL-like form usable by ThriveERP's ORM.
     *
     ...
     * Thrive's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
...
var Model = Class.extend({
    ...
    @param {String} name name of the ThriveERP model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound ThriveERP model.
     *
