thrive.define('web.mixins', function (require) {
"use strict";
...
function ThriveEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
ThriveEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

ThriveEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * ThriveERP Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    trigger_up: function (name, info) {
        var event = new ThriveEvent(this, name, info);
