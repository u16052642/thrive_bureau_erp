thrive.define('web.BasicController', function (require) {
"use strict";
...
var BasicController = AbstractController.extend(FieldManagerMixin, {
    ...
    init: function (parent, model, renderer, params) {
        ...
        // record before leaving Thrive (see @_urgentSave).
        this.pendingChanges = [];
    },
    /**
     * @override
     ...
     * haven't been sent to the model yet, just in case we would leave Thrive
     ...
     * @param {ThriveEvent} e - the event that triggered the change
     * @returns {Promise}
     */
    ...
    /**
     * To be called **only** when Thrive is about to be closed, and we want to
    ...
    _onContentClicked(ev) {
        if (this.$buttons && this._shouldBounceOnClick(ev.target)) {
            this.$buttons.find('.btn-primary:visible:first').thriveBounce();
        }
    },
    /**
