...
QUnit.module("documents_spreadsheet > chart side panel", { beforeEach, afterEach }, () => {
    ...
    QUnit.test("From an Thrive chart, can only change to an Thrive chart", async (assert) => {
        const { model, env } = await createSpreadsheetFromGraphView();
        await openChartSidePanel(model, env);
        const target = getFixture();
        ...
        assert.strictEqual(options[0].value, "thrive_bar");
        assert.strictEqual(options[1].value, "thrive_line");
        assert.strictEqual(options[2].value, "thrive_pie");
    });
    ...
    QUnit.test("Change thrive chart type", async (assert) => {
        const { model, env } = await createSpreadsheetFromGraphView();
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
        assert.strictEqual(model.getters.getChart(chartId).type, "thrive_bar");
        await openChartSidePanel(model, env);
        const target = getFixture();
        /** @type {HTMLSelectElement} */
        const select = target.querySelector(".o-type-selector");
        select.value = "thrive_pie";
        await triggerEvent(select, null, "change");
        assert.strictEqual(model.getters.getChart(chartId).type, "thrive_pie");
        select.value = "thrive_line";
        await triggerEvent(select, null, "change");
        assert.strictEqual(model.getters.getChart(chartId).type, "thrive_line");
