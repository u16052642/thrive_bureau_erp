    ...
    function () {
        QUnit.test("Reinsert a pivot", async function (assert) {
            ...
            assert.equal(
                ...
                `=THRIVE.PIVOT(1,"probability","bar","false","foo",1)`,
        ...
        QUnit.test("Reinsert a pivot with new data", async function (assert) {
            ...
            selectCell(model, "D8");
            await doMenuAction(topbarMenuRegistry, reinsertPivotPath, env);
            assert.equal(getCellFormula(model, "I8"), `=THRIVE.PIVOT.HEADER(1,"foo",25)`);
            assert.equal(
                getCellFormula(model, "I10"),
                `=THRIVE.PIVOT(1,"probability","bar","false","foo",25)`
            );
        });
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                await doMenuAction(topbarMenuRegistry, reinsertPivotPath, env);
                assert.equal(getCellFormula(model, "C1"), `=THRIVE.PIVOT.HEADER(1,"foo",2)`);
                assert.equal(
                    getCellFormula(model, "C2"),
                    `=THRIVE.PIVOT.HEADER(1,"foo",2,"measure","probability")`
                );
                assert.equal(
        ...
        QUnit.test(
            "Can rebuild the Thrive domain of records based on the according merged pivot cell",
            async function (assert) {
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                setCellContent(
                    model,
                    "B4",
                    getCellFormula(model, "B4").replace("THRIVE.PIVOT", "thrive.pivot")
                );
                assert.ok(root.isVisible(env));
            }
        );
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                setCellContent(
                    model,
                    "B4",
                    getCellFormula(model, "B4").replace(`THRIVE.PIVOT(1`, `THRIVE.PIVOT("5)`)
                ); //Invalid id
