...
from thrive import api, Command, fields, models, tools, SUPERUSER_ID, _, _lt
from thrive.addons.rating.models import rating_data
from thrive.exceptions import UserError, ValidationError, AccessError
...
class Task(models.Model):
    ...
    def _notify_by_email_get_headers(self):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Thrive-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Thrive-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Thrive-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
