...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Thrive Bureau ERP database
        """
        server = helpers.get_thrive_server_url()
        if server:
            subject = helpers.read_file_first_line('thrive-subject.conf')
            if subject:
            ...
            _logger.warning('Thrive server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the thrive server with the updates
        """
