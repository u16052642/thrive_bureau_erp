...
export function usePartnerAutocomplete() {
    const keepLastThrive = new KeepLast();
    ...
    function autocomplete(value, isVAT = false) {
        value = value.trim();
        let thriveSuggestions = [];
        let clearbitSuggestions = [];
        return new Promise((resolve, reject) => {
            const thrivePromise = getThriveSuggestions(value, isVAT).then((suggestions) => {
                thriveSuggestions = suggestions;
            });
            ...
            const concatResults = () => {
                // Add Clearbit result with Thrive result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    const websites = thriveSuggestions.map((suggestion) => {
                    ...
                    clearbitSuggestions.forEach((suggestion) => {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            thriveSuggestions.push(suggestion);
                        }
                    });
                }

                thriveSuggestions = thriveSuggestions.filter((suggestion) => {
                    return !suggestion.ignored;
                });
                thriveSuggestions.forEach((suggestion) => {
                    delete suggestion.ignored;
                });
                return resolve(thriveSuggestions);
            };

            whenAll([thrivePromise, clearbitPromise]).then(concatResults, concatResults);
        });
    }
    ...
    /**
     * Use Thrive Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    async function getThriveSuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastThrive.add(prom);
