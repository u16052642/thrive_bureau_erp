...
const HtmlFieldPatch = {
    ...
    async updateBehaviors(behaviorsData = [], target = null) {
        ...
        for (const behaviorData of behaviorsData) {
            ...
            if (!anchor.oKnowledgeBehavior) {
                if (!this.props.readonly && this.wysiwyg && this.wysiwyg.thriveEditor) {
                    this.wysiwyg.thriveEditor.observerUnactive('injectBehavior');
                }
                ...
                if (!this.props.readonly && this.wysiwyg && this.wysiwyg.thriveEditor) {
                    this.wysiwyg.thriveEditor.idSet(anchor);
                    this.wysiwyg.thriveEditor.observerActive('injectBehavior');
                    ...
                    this.wysiwyg.thriveEditor.historyStep();
                }
    ...
    _addRefreshBehaviorsListeners() {
        if (this.wysiwyg.thriveEditor) {
            this.wysiwyg.thriveEditor.addEventListener('historyUndo', this.bindedDelayedRefreshBehaviors);
            this.wysiwyg.thriveEditor.addEventListener('historyRedo', this.bindedDelayedRefreshBehaviors);
        }
    ...
    _removeRefreshBehaviorsListeners() {
        if (this.wysiwyg.thriveEditor) {
            this.wysiwyg.thriveEditor.removeEventListener('historyUndo', this.bindedDelayedRefreshBehaviors);
            this.wysiwyg.thriveEditor.removeEventListener('historyRedo', this.bindedDelayedRefreshBehaviors);
        }
        if (this.wysiwyg.$editable.length) {
