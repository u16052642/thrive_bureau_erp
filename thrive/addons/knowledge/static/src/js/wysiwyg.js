...
import { preserveCursor } from '@web_editor/js/editor/thrive-editor/src/ThriveEditor';
...
Wysiwyg.include({
     ...
     * block is destined to be used in @see ThriveEditor in modules other than
    ...
    _insertTableOfContent: function () {
        ...
        const [container] = this.thriveEditor.execCommand('insert', tableOfContentBlock);
    ...
    _insertArticlesStructure: function (childrenOnly) {
        ...
        const [container] = this.thriveEditor.execCommand('insert', articlesStructureBlock);
        this._notifyNewBehavior(container);
    },
    /**
    ...
    _insertTemplate() {
        ...
        const [container] = this.thriveEditor.execCommand('insert', templateBlock);
        this._notifyNewBehavior(container);
    },
    /**
    ...
    _insertArticleLink: function () {
        ...
        dialog.on('save', this, article => {
            if (article) {
                ...
                dialog.close();
                restoreSelection();
                const [anchor] = this.thriveEditor.execCommand('insert', articleLinkBlock);
    ...
    _insertEmbeddedView: async function (actWindowId, viewType, name, context={}) {
        ...
        const [container] = this.thriveEditor.execCommand('insert', embeddedViewBlock);
        this._notifyNewBehavior(container);
    },
    /**
    ...
    _onMediaDialogSave(params, element) {
        if (element.classList.contains('o_is_knowledge_file')) {
            ...
            const [container] = this.thriveEditor.execCommand('insert', fileBlock);
            this._notifyNewBehavior(container, {
