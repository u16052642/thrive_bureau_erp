...
export class IoTRestartThriveOrReboot extends Component {
    setup() {
        super.setup();
    ...
    async onClick() {
        this.dialog.add(ConfirmationDialog, {
            body:
                this.props.action == "restart_thrive"
                    ? this.env._t("Are you sure you want to restart Thrive on the IoT box?")
                    : this.env._t("Are you sure you want to reboot the IoT box?"),
            confirm: () => this.restartThriveOrReboot(),
            cancel: () => {},
        });
    }
    ...
    async callRestartMethodOnServer() {
        /// Call "restart_thrive_or_reboot" method from "hw_posbox_homepage" controller
        let restartResponse;
        try {
            ...
            restartResponse = await this.rpc(this.ip_url + "/iot_restart_thrive_or_reboot", {
...
IoTRestartThriveOrReboot.template = `iot.IoTRestartThriveOrReboot`;

IoTRestartThriveOrReboot.props = {
    ...standardWidgetProps,
    btn_name: { type: String },
    action: { type: String },
};

IoTRestartThriveOrReboot.extractProps = ({ field, attrs }) => {
    return {
...
registry.category("view_widgets").add("iot_restart_thrive_or_reboot", IoTRestartThriveOrReboot);
