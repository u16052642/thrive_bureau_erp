...
from thrive import _, models, Command
from thrive.tools import float_repr
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_fill_invoice_line_values(self, tree, xpath_dict, invoice_line, qty_factor):
        """
        Read the xml invoice, extract the invoice line values, compute the thrive values
        ...
        It is not possible to do it in Thrive.
        ...
        if quantity_node is not None:
            ...
            if uom_xml:
                uom_infered_xmlid = [
                    thrive_xmlid for thrive_xmlid, uom_unece in UOM_TO_UNECE_CODE.items() if uom_unece == uom_xml
                ]
