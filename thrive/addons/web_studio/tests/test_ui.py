...
import thrive.tests
from thrive import Command, api, http
from thrive.tools import mute_logger
from thrive.addons.web_studio.controllers.main import WebStudioController
from lxml import etree
...
def _get_studio_view(view):
    domain = [('inherit_id', '=', view.id), ('name', '=', "Thrive Studio: %s customization" % (view.name))]
...
@thrive.tests.tagged('post_install', '-at_install')
class TestStudioUIUnit(thrive.tests.HttpCase):
    ...
    def test_edit_with_xml_editor(self):
        studioView = self.env["ir.ui.view"].create({
            ...
            'name': "Thrive Studio: %s customization" % (self.testView.name)
        })
