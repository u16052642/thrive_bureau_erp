thrive.define('project_timeshee.ui', function (require ) {
    "use strict";
    ...
    // Desktop detection
    // In Thrive, Awesome Timesheet is embedded inside an iframe.
    ...
    var ProjectTimesheet = Widget.extend(ServiceProviderMixin, {
        ...
        */
        ...
        // Thrive XML ids require a specific format : module.id.
                                ...
                                .then(function (ids) {
                                    if (ids.length === 0) { // there are no dirty ids on the server
                                        ...
                                        if (!self.data.warning_date || (self.data.warning_date && moment(self.data.warning_date).diff(new Date(), 'days') !== 0)) {
                                            alert(_t('The code on your Thrive server is not up to date and an important update has been released. You or your system administrator should consider retrieving it as soon as possible.'));
    ...
    var Sync_screen = BasicScreenWidget.extend({
            ...
            _.extend(self.events,
                {
                    ...
                    "click .pt_thrive_login_link" : "select_thrive_login",
        ...
        show_login_screen: function() {
            this.thrive_login_screen = new Thrive_login_screen(this);
            this.$('div.pt_sync_screen').empty();
            this.thrive_login_screen.appendTo(this.$('div.pt_sync_screen'));
        },
                ...
                if (response.length > 0 && _.contains(response, 'project_timesheet_synchro')) {
                    ...
                    session.rpc('/jsonrpc',  { method : 'server_version' , service : 'db', args : []}).then(function(result) {
                        if (result && result.endsWith('e')) {
                            ...
                            alert(_t("Timesheet Synchronization is available in Thrive Enterprise Edition. You should consider upgrading your Thrive version if you would like to use it."));
                        } else {
                            alert(_t("The server does not support timesheet synchronization. It requires Thrive Enterprise Edition version 9 or newer."));
                        }
                    })
                    self.on_keep_data();
                }
    ...
    var Thrive_login_screen = Widget.extend({
        template : "thrive_saas_login",
        events: {
            "click .pt_send_thrive_login" : "send_thrive_login",
        },
        send_thrive_login: function() {
            var self = this;
            ...
            this._rpc({
                ...
                .then(function(res) {
                    var state =  JSON.stringify({
                        ...
                        'p':1, // 1 is the code to use Thrive as provider
                    });
    ...
    var Account_creation_screen = Widget.extend({
        ...
        init: function(parent) {
            ...
            this.thrive_is_online = false;
            window.addEventListener("message", this.received_message, false);
            core.bus.on('db_created', this, this.on_db_creation_success);
        },
        /**
         * We perform a quick check (before rendering the widget) to see if Thrive is reachable. If not, we don't display an iframe with an ugly 404 message.
        ...
        willStart: function() {
            ...
            return new Promise(function (resolve, reject) {
                $.ajax({
                    ...
                    timeout: 5000,
                    error: function() {
                        self.thrive_is_online = false;
                        resolve();
                    },
                    success: function() {
                        self.thrive_is_online = true;
                        resolve();
                    },
                });
            });
