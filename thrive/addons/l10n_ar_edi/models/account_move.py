...
from thrive.tools.float_utils import float_repr, float_round
from thrive.tools import html2plaintext, plaintext2html
...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for inv in sale_ar_edi_invoices:
            ...
            # in Thrive for consistency, this way if an error ocurrs later in another invoice we will have the ones
            # correctly validated in AFIP in Thrive (CAE, Result, xml response/request).
...
""",
    ...
    def _l10n_ar_check_afip_auth_verify_required(self):
        ...
        if still_missing:
            ...
            raise UserError(_('We can not post %s in Thrive because the AFIP verification fail: %s\nPlease verify in AFIP manually'
