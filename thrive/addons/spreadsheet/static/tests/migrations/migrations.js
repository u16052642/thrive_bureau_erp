...
import { migrate, THRIVE_VERSION } from "@spreadsheet/o_spreadsheet/migration";
...
QUnit.test("Thrive formulas are migrated", (assert) => {
    const data = {
        sheets: [
            {
                cells: {
    ...
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=THRIVE.PIVOT("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A2.content, `=THRIVE.PIVOT.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=THRIVE.FILTER.VALUE("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=THRIVE.LIST("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A5.content, `=THRIVE.LIST.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A6.content, `=THRIVE.PIVOT.POSITION("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A7.content, `=THRIVE.PIVOT("1")`);
});
...
QUnit.test("Pivot 'day' arguments are migrated", (assert) => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=THRIVE.PIVOT("1","21/07/2022")` },
                    A2: { content: `=THRIVE.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=thrive.pivot("1","21/07/2021")` },
                    A4: { content: `=THRIVE.PIVOT("1","test")` },
                    A5: { content: `=thrive.pivot("1","21/07/2021")+"21/07/2021"` },
    ...
    const migratedData = migrate(data);
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=THRIVE.PIVOT("1","07/21/2022")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A2.content,
        `=THRIVE.PIVOT.HEADER("1","12/11/2022")`
    );
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=thrive.pivot("1","07/21/2021")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=THRIVE.PIVOT("1","test")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A5.content,
        `=thrive.pivot("1","07/21/2021")+"21/07/2021"`
    );
...
QUnit.test("Thrive version is exported", (assert) => {
    const model = new Model();
    assert.strictEqual(model.exportData().thriveVersion, THRIVE_VERSION);
});
