    ...
    () => {
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.equal(
                    exportedData.chartThriveMenusReferences[chartId],
                    1,
                    "Link to thrive menu is exported"
                );
                ...
                assert.equal(chartMenu.id, 1, "Link to thrive menu is imported");
            }
        );

        QUnit.test("Can undo-redo a LINK_THRIVE_MENU_TO_CHART", async function (assert) {
            const env = await makeTestEnv({ serverData: this.serverData });
            const model = new Model({}, { evalContext: { env } });
            createBasicChart(model, chartId);
            model.dispatch("LINK_THRIVE_MENU_TO_CHART", {
            ...
            assert.equal(model.getters.getChartThriveMenu(chartId).id, 1);
            model.dispatch("REQUEST_UNDO");
            assert.equal(model.getters.getChartThriveMenu(chartId), undefined);
