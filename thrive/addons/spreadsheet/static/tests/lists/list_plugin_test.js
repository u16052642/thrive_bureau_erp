...
QUnit.module("spreadsheet > list plugin", {}, () => {
    QUnit.test("List export", async (assert) => {
        ...
        assert.strictEqual(getCellFormula(model, "A1"), `=THRIVE.LIST.HEADER(1,"foo")`);
        assert.strictEqual(getCellFormula(model, "B1"), `=THRIVE.LIST.HEADER(1,"bar")`);
        assert.strictEqual(getCellFormula(model, "C1"), `=THRIVE.LIST.HEADER(1,"date")`);
        assert.strictEqual(getCellFormula(model, "D1"), `=THRIVE.LIST.HEADER(1,"product_id")`);
        assert.strictEqual(getCellFormula(model, "A2"), `=THRIVE.LIST(1,1,"foo")`);
        assert.strictEqual(getCellFormula(model, "B2"), `=THRIVE.LIST(1,1,"bar")`);
        assert.strictEqual(getCellFormula(model, "C2"), `=THRIVE.LIST(1,1,"date")`);
        assert.strictEqual(getCellFormula(model, "D2"), `=THRIVE.LIST(1,1,"product_id")`);
        assert.strictEqual(getCellFormula(model, "A3"), `=THRIVE.LIST(1,2,"foo")`);
        assert.strictEqual(getCellFormula(model, "A11"), `=THRIVE.LIST(1,10,"foo")`);
    ...
    QUnit.test("Can display a field which is not in the columns", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        setCellContent(model, "A1", `=THRIVE.LIST(1,1,"active")`);
    ...
    QUnit.test("Can remove a list with undo after editing a cell", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        assert.ok(getCellContent(model, "B1").startsWith("=THRIVE.LIST.HEADER"));
    ...
    QUnit.test("can select a List from cell formula", async function (assert) {
        ...
        model.dispatch("SELECT_THRIVE_LIST", { listId });
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithList();
            setCellContent(model, "A1", `=-THRIVE.LIST("1","1","foo")`);
            const sheetId = model.getters.getActiveSheetId();
            const listId = model.getters.getListIdFromPosition(sheetId, 0, 0);
            model.dispatch("SELECT_THRIVE_LIST", { listId });
            const selectedListId = model.getters.getSelectedListId();
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithList();
            setCellContent(model, "A1", `=3*THRIVE.LIST("1","1","foo")`);
            const sheetId = model.getters.getActiveSheetId();
            const listId = model.getters.getListIdFromPosition(sheetId, 0, 0);
            model.dispatch("SELECT_THRIVE_LIST", { listId });
            const selectedListId = model.getters.getSelectedListId();
    ...
    QUnit.test("can select a List from cell formula within a formula", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        setCellContent(model, "A1", `=SUM(THRIVE.LIST("1","1","foo"),1)`);
        const sheetId = model.getters.getActiveSheetId();
        const listId = model.getters.getListIdFromPosition(sheetId, 0, 0);
        model.dispatch("SELECT_THRIVE_LIST", { listId });
        const selectedListId = model.getters.getSelectedListId();
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithList();
            setCellContent(model, "A1", `=THRIVE.LIST(G10,"1","foo")`);
    ...
    QUnit.test("Referencing non-existing fields does not crash", async function (assert) {
        ...
        await nextTick();
        setCellContent(model, "A1", `=THRIVE.LIST.HEADER("1", "${forbiddenFieldName}")`);
        setCellContent(model, "A2", `=THRIVE.LIST("1","1","${forbiddenFieldName}")`);
        ...
        const spreadsheetData = {
            sheets: [
                ...
                },
                {
                    id: "sheet2",
                    cells: {
                        A1: { content: `=THRIVE.LIST("1", "1", "foo")` },
                    },
                },
            ],
            lists: {
            ...
            sheets: [
                {
                    id: "sheet1",
                    cells: {
                        A1: { content: `=THRIVE.LIST("1", "1", "name")` },
                    },
                },
            ],
            lists: {
    ...
    QUnit.test("rename list with empty name is refused", async (assert) => {
        const { model } = await createSpreadsheetWithList();
        const result = model.dispatch("RENAME_THRIVE_LIST", {
            listId: "1",
            name: "",
        });
    ...
    QUnit.test("Undo/Redo for RENAME_THRIVE_LIST", async function (assert) {
        assert.expect(4);
        const { model } = await createSpreadsheetWithList();
        assert.equal(model.getters.getListName("1"), "List");
        model.dispatch("RENAME_THRIVE_LIST", { listId: "1", name: "test" });
    ...
    QUnit.test("Can delete list", async function (assert) {
        const { model } = await createSpreadsheetWithList();
        model.dispatch("REMOVE_THRIVE_LIST", { listId: "1" });
    ...
    QUnit.test("can edit list domain", async (assert) => {
        ...
        model.dispatch("UPDATE_THRIVE_LIST_DOMAIN", {
            listId,
            domain: [["foo", "in", [55]]],
        });
