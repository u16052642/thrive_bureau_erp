...
QUnit.module("spreadsheet > pivot_helpers", {}, () => {
    QUnit.test("Basic formula extractor", async function (assert) {
        const formula = `=THRIVE.PIVOT("1", "test") + THRIVE.LIST("2", "hello", "bla")`;
        let functionName;
        let args;
        ({ functionName, args } = getFirstPivotFunction(formula));
        assert.strictEqual(functionName, "THRIVE.PIVOT");
        ...
        assert.strictEqual(functionName, "THRIVE.LIST");
    ...
    QUnit.test("Extraction with two PIVOT formulas", async function (assert) {
        const formula = `=THRIVE.PIVOT("1", "test") + THRIVE.PIVOT("2", "hello", "bla")`;
        let functionName;
        let args;
        ({ functionName, args } = getFirstPivotFunction(formula));
        assert.strictEqual(functionName, "THRIVE.PIVOT");
    ...
    QUnit.test("Number of formulas", async function (assert) {
        const formula = `=THRIVE.PIVOT("1", "test") + THRIVE.PIVOT("2", "hello", "bla") + THRIVE.LIST("1", "bla")`;
