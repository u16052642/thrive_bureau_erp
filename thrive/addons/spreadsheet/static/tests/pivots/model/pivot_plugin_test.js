...
QUnit.module("spreadsheet > pivot plugin", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithPivot();
            setCellContent(model, "C3", `=THRIVE.PIVOT(G10,"probability","bar","false","foo","2")+2`);
    ...
    QUnit.test("Can remove a pivot with undo after editing a cell", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.ok(getCellContent(model, "B1").startsWith("=THRIVE.PIVOT.HEADER"));
    ...
    QUnit.test("rename pivot with empty name is refused", async (assert) => {
        const { model } = await createSpreadsheetWithPivot();
        const result = model.dispatch("RENAME_THRIVE_PIVOT", {
            pivotId: "1",
            name: "",
        });
    ...
    QUnit.test("Undo/Redo for RENAME_THRIVE_PIVOT", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.equal(model.getters.getPivotName("1"), "Partner Pivot");
        model.dispatch("RENAME_THRIVE_PIVOT", { pivotId: "1", name: "test" });
    ...
    QUnit.test("Format header displays an error for non-existing field", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "G10", `=THRIVE.PIVOT.HEADER("1", "measure", "non-existing")`);
        setCellContent(model, "G11", `=THRIVE.PIVOT.HEADER("1", "non-existing", "bla")`);
        await nextTick();
            ...
            sheets: [
                {
                    ...
                    cells: {
                        A1: { content: `=THRIVE.PIVOT(1, "probability")` },
                        A2: { content: `=THRIVE.PIVOT(2, "probability")` },
                    },
                },
            ],
            pivots: {
            ...
            sheets: [
                {
                    id: "sheet1",
                    cells: {
                        A1: { content: `=THRIVE.PIVOT.HEADER(1, "measure", "probability")` },
                        A2: { content: `=THRIVE.PIVOT.HEADER(1, "product_id", 37)` },
                        A3: { content: `=THRIVE.PIVOT(1, "probability")` },
                    },
                },
            ],
            pivots: {
    ...
    QUnit.test("relational PIVOT.HEADER with missing id", async function (assert) {
        ...
        model.dispatch("UPDATE_CELL", {
            col: 4,
            row: 9,
            content: `=THRIVE.PIVOT.HEADER("1", "product_id", "1111111")`,
            sheetId,
        });
        await waitForDataSourcesLoaded(model);
        assert.equal(
    ...
    QUnit.test("relational PIVOT.HEADER with undefined id", async function (assert) {
        ...
        setCellContent(model, "F10", `=THRIVE.PIVOT.HEADER("1", "product_id", A25)`);
    ...
    QUnit.test("Can group by many2many field ", async (assert) => {
        ...
        assert.equal(getCellFormula(model, "A3"), '=THRIVE.PIVOT.HEADER(1,"tag_ids","false")');
        assert.equal(getCellFormula(model, "A4"), '=THRIVE.PIVOT.HEADER(1,"tag_ids",42)');
        assert.equal(getCellFormula(model, "A5"), '=THRIVE.PIVOT.HEADER(1,"tag_ids",67)');

        assert.equal(
            getCellFormula(model, "B3"),
            '=THRIVE.PIVOT(1,"probability","tag_ids","false","foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B4"),
            '=THRIVE.PIVOT(1,"probability","tag_ids",42,"foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B5"),
            '=THRIVE.PIVOT(1,"probability","tag_ids",67,"foo",1)'
        );

        assert.equal(
            getCellFormula(model, "C3"),
            '=THRIVE.PIVOT(1,"probability","tag_ids","false","foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C4"),
            '=THRIVE.PIVOT(1,"probability","tag_ids",42,"foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C5"),
            '=THRIVE.PIVOT(1,"probability","tag_ids",67,"foo",2)'
        );
    ...
    QUnit.test("can edit pivot domain", async (assert) => {
        ...
        model.dispatch("UPDATE_THRIVE_PIVOT_DOMAIN", {
            pivotId,
            domain: [["foo", "in", [55]]],
        });
