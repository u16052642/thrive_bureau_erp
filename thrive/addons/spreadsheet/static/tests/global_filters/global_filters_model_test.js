...
QUnit.module("spreadsheet > Global filters model", {}, () => {
    ...
    QUnit.test("Domain of simple date filter", async function (assert) {
        ...
        const chartId = model.getters.getThriveChartIds()[0];
        await addGlobalFilter(
            model,
            {
                filter: {
    ...
    QUnit.test("THRIVE.FILTER.VALUE text filter", async function (assert) {
        assert.expect(3);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=THRIVE.FILTER.VALUE("Text Filter")`);
        await nextTick();
    ...
    QUnit.test("THRIVE.FILTER.VALUE date filter", async function (assert) {
        assert.expect(4);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=THRIVE.FILTER.VALUE("Date Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            filter: {
                id: "42",
    ...
    QUnit.test("THRIVE.FILTER.VALUE relation filter", async function (assert) {
        assert.expect(6);
        ...
        setCellContent(model, "A10", `=THRIVE.FILTER.VALUE("Relation Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            filter: {
                id: "42",
    ...
    QUnit.test(
        "THRIVE.FILTER.VALUE formulas are updated when filter label is changed",
        async function (assert) {
            ...
            setCellContent(
                model,
                "A10",
                `=THRIVE.FILTER.VALUE("Cuillère") & THRIVE.FILTER.VALUE( "Cuillère" )`
            );
            const [filter] = model.getters.getGlobalFilters();
            const newFilter = {
                type: "date",
            ...
            assert.equal(
                ...
                `=THRIVE.FILTER.VALUE("Interprete") & THRIVE.FILTER.VALUE("Interprete")`
            );
        }
    );
                ...
                sheets: [
                    {
                        id: "sheet1",
                        cells: {
                            A1: { content: `=THRIVE.PIVOT("1", "probability")` },
                        },
                    },
                ],
                pivots: {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const dateFilters2 = model.getters.getFiltersMatchingPivot(
                '=THRIVE.PIVOT.HEADER(1,"date:year","2016")'
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const filters = model.getters.getFiltersMatchingPivot(
                '=THRIVE.PIVOT.HEADER(1,"date:month","08/2016","product_id","41")'
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=THRIVE.PIVOT(1, "__count", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    filter: {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=THRIVE.PIVOT(1, "probability", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    filter: {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=THRIVE.PIVOT(1, "probability")');
            await addGlobalFilter(
                model,
                {
                    filter: {
    ...
    QUnit.test("field matching is removed when list is deleted", async function (assert) {
        ...
        model.dispatch("REMOVE_THRIVE_LIST", { listId });
        assert.throws(
    ...
    QUnit.test("field matching is removed when an Thrive chart is deleted", async function (assert) {
        ...
        assert.deepEqual(model.getters.getThriveChartFieldMatching(chartId, filter.id), matching);
        model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
        assert.throws(
            () => model.getters.getThriveChartFieldMatching(chartId, filter.id),
