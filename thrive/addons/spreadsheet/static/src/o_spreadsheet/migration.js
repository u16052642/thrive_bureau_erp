...
export const THRIVE_VERSION = 5;

const MAP = {
    PIVOT: "THRIVE.PIVOT",
    "PIVOT.HEADER": "THRIVE.PIVOT.HEADER",
    "PIVOT.POSITION": "THRIVE.PIVOT.POSITION",
    "FILTER.VALUE": "THRIVE.FILTER.VALUE",
    LIST: "THRIVE.LIST",
    "LIST.HEADER": "THRIVE.LIST.HEADER",
};
...
export function migrate(data) {
    let _data = load(data);
    const version = _data.thriveVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["THRIVE.PIVOT", "THRIVE.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
export default class ThriveVersion extends CorePlugin {
    export(data) {
        data.thriveVersion = THRIVE_VERSION;
    }
}

ThriveVersion.getters = [];

corePluginRegistry.add("thriveMigration", ThriveVersion);
