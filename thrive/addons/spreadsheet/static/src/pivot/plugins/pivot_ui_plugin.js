...
export default class PivotUIPlugin extends spreadsheet.UIPlugin {
    ...
    handleEvent(event) {
        ...
        switch (event.type) {
            case "ZonesSelected": {
                ...
                if (cell !== undefined && cell.content.startsWith("=THRIVE.PIVOT.HEADER(")) {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "SELECT_PIVOT":
                ...
                break;
            case "REFRESH_PIVOT":
                this._refreshThrivePivot(cmd.id);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshThrivePivots();
                break;
     ...
     */
    _refreshThrivePivots() {
        for (const pivotId of this.getters.getPivotIds()) {
            this._refreshThrivePivot(pivotId, false);
        }
    }
