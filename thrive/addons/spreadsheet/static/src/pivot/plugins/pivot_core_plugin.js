...
export default class PivotCorePlugin extends CorePlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "UPDATE_THRIVE_PIVOT_DOMAIN": {
                this.history.update(
                    "pivots",
                    cmd.pivotId,
                    "definition",
            ...
            case "REDO": {
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) => cmd.type === "UPDATE_THRIVE_PIVOT_DOMAIN"
                );
    ...
    _insertColumns(sheetId, anchor, id, table) {
        ...
        for (const _row of table.getColHeaders()) {
            ...
            for (const cell of _row) {
                ...
                this._addPivotFormula(sheetId, anchorLeft, anchorTop, "THRIVE.PIVOT.HEADER", args);
    ...
    _insertRows(sheetId, anchor, id, table) {
        ...
        for (const row of table.getRowHeaders()) {
            ...
            this._addPivotFormula(sheetId, x, y, "THRIVE.PIVOT.HEADER", args);
            if (row.indent <= 2) {
    ...
    _insertBody(sheetId, anchor, id, table) {
        ...
        for (const col of table.getMeasureHeaders()) {
            ...
            for (const row of table.getRowHeaders()) {
                ...
                this._addPivotFormula(sheetId, x, y, "THRIVE.PIVOT", args);
                y++;
            }
            x++;
        }
