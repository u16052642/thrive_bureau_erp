        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) => cmd.type === "UPDATE_THRIVE_LIST_DOMAIN"
                );
    ...
    _insertHeaders(sheetId, anchor, id, columns) {
        ...
        for (const column of columns) {
            this.dispatch("UPDATE_CELL", {
                ...
                col,
                row,
                content: `=THRIVE.LIST.HEADER(${id},"${column.name}")`,
            });
            col++;
        }
        ...
        for (let i = 1; i <= linesNumber; i++) {
            ...
            for (const column of columns) {
                this.dispatch("UPDATE_CELL", {
                    ...
                    col,
                    row,
                    content: `=THRIVE.LIST(${id},${i},"${column.name}")`,
                });
                col++;
            }
            row++;
