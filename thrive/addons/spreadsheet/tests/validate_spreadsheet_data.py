...
xml_id_url_prefix = "thrive://ir_menu_xml_id/"

thrive_view_link_prefix = "thrive://view/"


def thrive_charts(data):
    """return all thrive chart definition in the spreadsheet"""
    figures = []
    for sheet in data["sheets"]:
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet["figures"]
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("thrive_")
        ]
    return figures
...
def thrive_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like thrive://view/{... view data...}
    """
    return [
        json.loads(url[len(thrive_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(thrive_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = thrive_charts(data)
    thrive_version = data.get("thriveVersion", 1)
    if thrive_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    all_fields = chain(
        ...
        extract_fields(chart_fields, thrive_charts(data)).items(),
        extract_fields(thrive_view_fields, thrive_view_links(data)).items(),
...
def xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartThriveMenusReferences", {}).values()) | {
