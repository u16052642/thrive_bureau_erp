...
REPO=https://github.com/thrive/thrive.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/thrive"
...
thrive/
thrive-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/thrive
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/thrive/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
