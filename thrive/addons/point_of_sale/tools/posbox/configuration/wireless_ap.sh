...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ -f "${WIFI_NETWORK_FILE}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/thrive/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		nginx -c /home/pi/thrive/addons/point_of_sale/tools/posbox/configuration/nginx_ap.conf

		service thrive restart
	fi
# wired
else
	killall nginx
