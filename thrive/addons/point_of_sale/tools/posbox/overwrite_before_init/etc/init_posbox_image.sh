...
wget 'https://nightly.thrivebureau.com/master/iotbox/eftdvs' -P /usr/local/bin/
chmod +x /usr/local/bin/eftdvs
wget 'https://nightly.thrivebureau.com/master/iotbox/eftapi.so' -P /usr/lib/

groupadd usbusers
usermod -a -G usbusers pi
usermod -a -G lp pi
usermod -a -G input lightdm
mkdir -v /var/log/thrive
chown pi:pi /var/log/thrive
chown pi:pi -R /home/pi/thrive/
...
echo "* * * * * rm /var/run/thrive/sessions/*" | crontab -
...
echo "addons/hw_drivers/iot_devices/" > /home/pi/thrive/.git/info/exclude
