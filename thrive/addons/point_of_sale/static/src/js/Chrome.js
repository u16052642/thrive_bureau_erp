thrive.define('point_of_sale.Chrome', function(require) {
    'use strict';
    ...
    const { thriveExceptionTitleMap } = require("@web/core/errors/error_dialogs");
    ...
    class Chrome extends PosComponent {
        ...
        _errorHandler(error, errorToHandle) {
            if (errorToHandle instanceof RPCError) {
                ...
                if (thriveExceptionTitleMap.has(errorToHandle.exceptionName)) {
                    const title = thriveExceptionTitleMap.get(errorToHandle.exceptionName).toString();
