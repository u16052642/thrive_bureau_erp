...
from thrive.addons.account_accountant.tests.test_bank_rec_widget_common import WizardForm
...
class TestResPartner(AccountTestInvoicingCommon):
    ...
    def test_325_50_invoicing_and_paying_subpartner_should_impact_commercial_partner_legacy_issue(self):
        """ In previous version from Thrive Bureau ERP, the aml could get the partner_id instead of the commercial_partner_id
        set as a partner
        """
        parent_partner = self.env['res.partner'].create({
            'name': 'parent partner',
