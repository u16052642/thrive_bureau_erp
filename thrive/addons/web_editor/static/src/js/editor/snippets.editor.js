thrive.define('web_editor.snippet.editor', function (require) {
'use strict';
...
var SnippetEditor = Widget.extend({
    ...
    removeSnippet: async function (shouldRecordUndo = true) {
        this.options.wysiwyg.thriveEditor.unbreakableStepUnactive();
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.thriveEditor.historyStep();
        }
    },
    /**
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.thriveEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _onDragAndDropStart: function () {
        ...
        this.options.wysiwyg.thriveEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
    ...
    _onDragAndDropStop: function (ev, ui) {
        this.options.wysiwyg.thriveEditor.automaticStepActive();
        this.options.wysiwyg.thriveEditor.automaticStepSkipStack();
        this.options.wysiwyg.thriveEditor.unbreakableStepUnactive();
        ...
        this.options.wysiwyg.thriveEditor.observerActive('dragAndDropMoveSnippet');
        if (this.dropped) {
            if (prev) {
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.thriveEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.thriveEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.thriveEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.thriveEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.thriveEditor.historyCanRedo());
                };
                this.options.wysiwyg.thriveEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.thriveEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.thriveEditor.editable).trigger('content_changed');
                });
                this.options.wysiwyg.thriveEditor.addEventListener('historyRevert', _.debounce(() => {
        ...
        const smoothScrollOptions = this._getScrollOptions({
            jQueryDraggableOptions: {
                ...
                start: function () {
                    ...
                    const doc = self.options.wysiwyg.thriveEditor.document;
                    $(doc.body).addClass('oe_dropzone_active');

                    self.options.wysiwyg.thriveEditor.automaticStepUnactive();

                    self.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                    self.options.wysiwyg.thriveEditor.observerUnactive('dragAndDropCreateSnippet');
                ...
                stop: async function (ev, ui) {
                    ...
                    self.options.wysiwyg.thriveEditor.automaticStepUnactive();
                    self.options.wysiwyg.thriveEditor.automaticStepSkipStack();
                    $toInsert.removeClass('oe_snippet_body');
                    ...
                    self.options.wysiwyg.thriveEditor.observerActive('dragAndDropCreateSnippet');
                    ...
                    if (dropped) {
                        ...
                        _.defer(async function () {
                            ...
                            self.options.wysiwyg.thriveEditor.unbreakableStepUnactive();
                            self.options.wysiwyg.thriveEditor.historyStep();
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onSnippetClick() {
        ...
        for (const el of $els) {
            if (!el.children.length) {
                $(el).thriveBounce('o_we_snippet_area_animation');
            }
        }
    },
    /**
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this.options.wysiwyg.thriveEditor.bindExecCommand($customizeTableBlock[0]);
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.thriveEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.thriveEditor.editable));
    },
    /**
