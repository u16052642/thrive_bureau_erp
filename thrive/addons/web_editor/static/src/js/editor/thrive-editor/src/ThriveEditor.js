...
export class ThriveEditor extends EventTarget {
    constructor(editable, options = {}) {
        ...
        this.editable.classList.add("thrive-editor-editable");
    ...
    _onClipboardCopy(clipboardEvent) {
        if (this.isSelectionInEditable()) {
            ...
            const thriveHtml = dataHtmlElement.innerHTML;
            const thriveText = dataHtmlElement.innerText;
            if (!clipboardEvent.clipboardData.getData('text/plain')) {
                clipboardEvent.clipboardData.setData('text/plain', thriveText);
            }
            if (!clipboardEvent.clipboardData.getData('text/html')) {
                clipboardEvent.clipboardData.setData('text/html', thriveHtml);
            }
            clipboardEvent.clipboardData.setData('text/thrive-editor', thriveHtml);
        }
    }
    /**
     * @private
    ...
    isSelectionInBlockRoot() {
        ...
        while (
            ...
            !currentNode.classList.contains('thrive-editor-editable') &&
    ...
    _onPaste(ev) {
        ...
        const thriveEditorHtml = ev.clipboardData.getData('text/thrive-editor');
        ...
        if (thriveEditorHtml && targetSupportsHtmlContent) {
            const fragment = parseHTML(thriveEditorHtml);
