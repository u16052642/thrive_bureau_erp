thrive.define('web_editor.snippets.options', function (require) {
'use strict';
...
const SnippetOptionWidget = Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.thriveEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.thriveEditor.automaticStepActive('preview_option');
        }
...
registry.sizing = SnippetOptionWidget.extend({
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ev.preventDefault();
            self.options.wysiwyg.thriveEditor.automaticStepUnactive('resizing');
            ...
            if (rowEl.classList.contains('o_grid_mode')) {
                self.options.wysiwyg.thriveEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                self.options.wysiwyg.thriveEditor.observerActive('displayBackgroundGrid');
            ...
            const bodyMouseUp = function () {
                ...
                self.options.wysiwyg.thriveEditor.automaticStepActive('resizing');
            };
...
￼    */
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Thrive).
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.thriveEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.thriveEditor.observerActive('_markColorLevel');
    },
});
