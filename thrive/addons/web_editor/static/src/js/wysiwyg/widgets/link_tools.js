thrive.define('wysiwyg.widgets.LinkTools', function (require) {
'use strict';
...
const LinkTools = Link.extend({
    ...
    _updateColorpicker: async function (cssProperty) {
        ...
        if (!colorpicker) {
            colorpicker = new ColorPaletteWidget(this, {
                ...
                $editable: $(this.options.wysiwyg.thriveEditor.editable),
            ...
            colorpicker.on('custom_color_picked color_picked color_hover color_leave', this, (ev) => {
                ...
                if (['custom_color_picked', 'color_picked'].includes(ev.name)) {
                    ...
                    this.options.wysiwyg.thriveEditor.execCommand('applyColor', '', 'color');
                    this.options.wysiwyg.thriveEditor.execCommand('applyColor', '', 'backgroundColor');
                }
     ...
     */
    _addHintClasses () {
        this.options.wysiwyg.thriveEditor.observerUnactive("hint_classes");
        this.$link.addClass('oe_edited_link');
        this.$button.addClass('active');
        this.options.wysiwyg.thriveEditor.observerActive("hint_classes");
    },
    /**
    ...
    _removeHintClasses () {
        this.options.wysiwyg.thriveEditor.observerUnactive("hint_classes");
        $(this.options.wysiwyg.thriveEditor.document).find('.oe_edited_link').removeClass('oe_edited_link');
    ...
    __onURLInput() {
        this._super(...arguments);
        this.options.wysiwyg.thriveEditor.historyPauseSteps('_onURLInput');
        this._adaptPreview();
        this.options.wysiwyg.thriveEditor.historyUnpauseSteps('_onURLInput');
    },
});
