...
const LinkPopoverWidget = Widget.extend({
    ...
    start() {
        ...
        .on('show.bs.popover.link_popover', () => {
            this.options.wysiwyg.thriveEditor.observerUnactive('show.bs.popover');
            this._loadAsyncLinkPreview();
            popoverShown = true;
        })
        .on('inserted.bs.popover', () => {
            this.options.wysiwyg.thriveEditor.observerActive('show.bs.popover');
        })
        .on('hide.bs.popover.link_popover', () => {
            this.options.wysiwyg.thriveEditor.observerUnactive('hide.bs.popover');
            popoverShown = false;
        })
        .on('hidden.bs.popover.link_popover', () => {
            this.options.wysiwyg.thriveEditor.observerActive('hide.bs.popover');
        ...
        if (document !== this.options.wysiwyg.thriveEditor.document) {
            $(this.options.wysiwyg.thriveEditor.document).on('mouseup.link_popover', onClickDocument);
        }
    ...
    destroy() {
        ...
        $(this.options.wysiwyg.thriveEditor.document).off('.link_popover');
...
LinkPopoverWidget.createFor = async function (parent, targetEl, options) {
    ...
    if (wysiwyg) {
        wysiwyg.thriveEditor.observerUnactive('LinkPopoverWidget');
    }
    await popoverWidget.appendTo(targetEl)
    if (wysiwyg) {
        wysiwyg.thriveEditor.observerActive('LinkPopoverWidget');
    }
