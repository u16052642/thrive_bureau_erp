thrive.define('web_editor.wysiwyg', function (require) {
'use strict';
...
const ThriveEditorLib = require('@web_editor/js/editor/thrive-editor/src/ThriveEditor');
...
const ThriveEditor = ThriveEditorLib.ThriveEditor;
const getDeepRange = ThriveEditorLib.getDeepRange;
const getInSelection = ThriveEditorLib.getInSelection;
const isBlock = ThriveEditorLib.isBlock;
const rgbToHex = ThriveEditorLib.rgbToHex;
const preserveCursor = ThriveEditorLib.preserveCursor;
const closestElement = ThriveEditorLib.closestElement;
const setSelection = ThriveEditorLib.setSelection;
const endPos = ThriveEditorLib.endPos;
const hasValidSelection = ThriveEditorLib.hasValidSelection;
const parseHTML = ThriveEditorLib.parseHTML;
...
const Wysiwyg = Widget.extend({
    ...
    startEdition: async function () {
        ...
        this.thriveEditor = new ThriveEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.thriveEditor.addEventListener('contentChanged', function () {
        ...
        if (this.thriveEditor.document !== document) {
            this.thriveEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.thriveEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.thriveEditor.document.addEventListener("keyup", this._signalOnline, true);
        }

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.thriveEditor.multiselectionRefresh, { passive: true });
        ...
        this._observeThriveFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.thriveEditor.observerUnactive();
                const $field = $(this);
                ...
                self.thriveEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, function () {
            if (this.isContentEditable || (this.parentElement && this.parentElement.isContentEditable)) {
                ...
                const selection = self.thriveEditor.document.getSelection();
        ...
        if (options.snippets) {
            $(this.thriveEditor.document.body).addClass('editor_enable');
        ...
        $(this.thriveEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.thriveEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.thriveEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.thriveEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        this.setCSSVariables(this.snippetsMenu ? this.snippetsMenu.el : this.toolbar.el);

        this.thriveEditor.addEventListener('preObserverActive', () => {
            ...
            this._setONotEditable(this.thriveEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.thriveEditor.isMobile) {
                $(this.thriveEditor.editable).before(this.toolbar.$el);
            } else {
    ...
    setupCollaboration(collaborationChannel) {
        ...
        this._getNewPtp = () => {
            ...
            return new PeerToPeer({
                ...
                onRequest: {
                    ...
                    get_missing_steps: (params) => this.thriveEditor.historyGetMissingSteps(params.requestPayload),
                    get_history_from_snapshot: () => this.thriveEditor.historyGetSnapshotSteps(),
                    get_collaborative_selection: () => this.thriveEditor.getCurrentCollaborativeSelection(),
                },
                onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                    switch (notificationName) {
                        case 'ptp_remove':
                            this.thriveEditor.multiselectionRemove(notificationPayload);
                            break;
                        ...
                        case 'ptp_disconnect':
                            ...
                            this.thriveEditor.multiselectionRemove(fromClientId);
                            break;
                        case 'rtc_data_channel_open': {
                            ...
                            if (!historySyncAtLeastOnce) {
                                ...
                                if (remoteSelection) {
                                    this.thriveEditor.onExternalMultiselectionUpdate(remoteSelection);
                                }
                            }
                            break;
                        }
                        case 'oe_history_step':
                            ...
                            if (historySyncFinished) {
                                this.thriveEditor.onExternalHistorySteps([notificationPayload]);
                            }
                            break;
                        case 'oe_history_set_selection': {
                            ...
                            this.thriveEditor.onExternalMultiselectionUpdate(selection);
                            break;
                        }
                    }
                }
        ...
        const editorCollaborationOptions = {
            ...
            onHistoryMissingParentSteps: async ({ step, fromStepId }) => {
                ...
                this.ptp && this.thriveEditor.onExternalHistorySteps(missingSteps.concat([step]));
            },
            onHistoryNeedSync: async () => {
                ...
                if (firstClientId !== this._currentClientId) {
                    ...
                    this.thriveEditor.historyResetFromSteps(historySteps);
    ...
    destroy: function () {
        ...
        if (this.thriveEditor) {
            this.thriveEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.thriveEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.thriveEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.thriveEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            for (const observer of this._oNotEditableObservers.values()) {
                observer.disconnect();
            }
            this.thriveEditor.destroy();
        }
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.thriveEditor.multiselectionRefresh, { passive: true });
        }
        $(this.$root).off('click');
        if (this.linkPopover) {
    ...
    renderElement: function () {
        ...
        if (this.options.resizable && !device.isMobile) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper thrive-editor">');
            this.$root = $wrapper;
    ...
    getValue: function (options) {
        ...
        this.thriveEditor && this.thriveEditor.cleanForSave($editable[0]);
    ...
    historyReset: function () {
        this.thriveEditor.historyReset();
    },
    /**
     * Saves the content.
     *
    ...
    cancel: function (reload) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.thriveEditor.historySize().length) {
                resolve();
            } else {
    ...
    setValue: function (value) {
        this.thriveEditor.resetContent(value);
    },
    /**
     * Undo one step of change in the editor.
     */
    undo: function () {
        this.thriveEditor.historyUndo();
    },
    /**
     * Redo one step of change in the editor.
     */
    redo: function () {
        this.thriveEditor.historyRedo();
    },
    /**
     ...
     */
    focus: function () {
        if (this.thriveEditor && !this.thriveEditor.historyResetLatestComputedSelection(true)) {
    ...
    getDeepRange() {
        return getDeepRange(this.thriveEditor.editable);
    },
    closestElement(...args) {
        return closestElement(...args);
    },
    async cleanForSave() {
        this.thriveEditor && this.thriveEditor.cleanForSave();
    ...
    isSelectionInEditable: function () {
        return this.thriveEditor.isSelectionInEditable();
    },
    /**
     * Start or resume the Thrive field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeThriveFieldChanges: function () {
        ...
        if (this.thriveFieldObservers) {
            for (let observerData of this.thriveFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const thriveFieldSelector = '[data-oe-model], [data-oe-translation-initial-sha]';
            const $thriveFields = this.$editable.find(thriveFieldSelector);
            this.thriveFieldObservers = [];

            $thriveFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.thriveEditor.filterMutationRecords(mutations);
                    if (!mutations.length) {
                        return;
                    }
                    let $node = $(field);
                    let $nodes = $thriveFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    this._pauseThriveFieldObservers();
                ...
                this.thriveFieldObservers.push({field: field, observer: observer});
            });
        }
    },
    /**
     ...
     */
    _pauseThriveFieldObservers: function () {
        for (let observerData of this.thriveFieldObservers) {
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.thriveEditor.document.getSelection().getRangeAt(0).startContainer;
    ...
    toggleLinkTools(options = {}) {
        const linkEl = getInSelection(this.thriveEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.linkTools) {
                ...
                if (!this.linkTools || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkTools.$link[0])) {
                    const { link } = Link.getOrCreateLink({
                        containerNode: this.thriveEditor.editable,
                    ...
                    this.linkTools = new weWidgets.LinkTools(this, {wysiwyg: this, noFocusUrl: options.noFocusUrl}, this.thriveEditor.editable, linkToolsData, $btn, link );
                }
                ...
                const _onClick = ev => {
                    if (
                        ...
                        this.thriveEditor.document.removeEventListener('click', _onClick, true);
                        document.removeEventListener('click', _onClick, true);
                    }
                };
                this.thriveEditor.document.addEventListener('click', _onClick, true);
            ...
            const restoreSelection = preserveCursor(this.thriveEditor.document);
            linkDialog.open();
            linkDialog.on('save', this, data => {
                ...
                this.thriveEditor.setContenteditableLink(linkWidget.$link[0]);
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            ...
            this.thriveEditor.execCommand('unlink');
        }
    },
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.thriveEditor.document.getSelection();
        ...
        const $editable = $(ThriveEditorLib.closestElement(range.startContainer, '.o_editable') || this.thriveEditor.editable);
    ...
    _onMediaDialogSave: function (params, element) {
        ...
        if (params.node) {
            ...
            this.thriveEditor.unbreakableStepUnactive();
            this.thriveEditor.historyStep();
        } else {
            return this.thriveEditor.execCommand('insert', element);
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.thriveEditor.document, selector);
    },
    /**
    ...
    _configureToolbar: function (options) {
        ...
        $toolbar.find('#image-crop').click(e => {
            ...
            new weWidgets.ImageCropWidget(this, this.lastMediaClicked).appendTo(this.thriveEditor.document.body);
            this.thriveEditor.toolbarHide();
            $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.thriveEditor.toolbarShow());
        });
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.thriveEditor.document});
            const mouseup = () => {
                $('#image-transform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.thriveEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.thriveEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
            ...
            $(this.thriveEditor.document).on('mousedown', mousedown);
        });
        ...
        setTimeout(() => {
            ...
            if (!options.snippets && scrollableContainer.length) {
                this.thriveEditor.addDomListener(
                    scrollableContainer[0],
                    'scroll',
                    this.thriveEditor.updateToolbarPosition.bind(this.thriveEditor),
                );
            }
        }, 0);
    },
        ...
        for (const elem of $dropdownContent) {
            ...
            $dropdown.on('show.bs.dropdown', () => {
                ...
                mutex.exec(() => {
                    ...
                    this.thriveEditor.historyStash();
                    colorpicker = new ColorPaletteWidget(this, {
                        excluded: ['transparent_grayscale'],
                        $editable: $(this.thriveEditor.editable), // Our parent is the root widget, we can't retrieve the editable section from it...
                    ...
                    colorpicker.on('custom_color_picked color_picked', null, ev => {
                        if (hadNonCollapsedSelection) {
                            this.thriveEditor.historyResetLatestComputedSelection(true);
                        }
                        // Unstash the mutations now that the color is picked.
                        this.thriveEditor.historyUnstash();
                        ...
                        // hides it).
                        if (this.thriveEditor.deselectTable() && hasValidSelection(this.thriveEditor.editable)) {
                            this.thriveEditor.document.getSelection().collapseToStart();
                        }
                    ...
                    colorpicker.on('color_hover', null, ev => {
                        ...
                        this.thriveEditor.historyPauseSteps();
                        try {
                            this._processAndApplyColor(eventName, ev.data.color, true);
                        } finally {
                            this.thriveEditor.historyUnpauseSteps();
                        }
                    });
                    colorpicker.on('color_leave', null, ev => {
                        this.thriveEditor.historyRevertCurrentStep();
                    });
    ...
    _processAndApplyColor: function (eventName, color, previewMode) {
        ...
        const coloredElements = this.thriveEditor.execCommand('applyColor', color, eventName === 'foreColor' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        ...
        this.thriveEditor.updateColorpickerLabels({
    ...
    _updateEditorUI: function (e) {
        let selection = this.thriveEditor.document.getSelection();
        const anchorNode = selection.anchorNode;
        if (anchorNode && closestElement(anchorNode, '[data-oe-protected="true"]')) {
            return;
        }

        this.thriveEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.thriveEditor.document.defaultView;
        ...
        selection = this.thriveEditor.document.getSelection();
        ...
        if (isInMedia) {
            ...
            const selection = this.thriveEditor.document.getSelection();
            const range = this.thriveEditor.document.createRange();
        ...
        const link = getInSelection(this.thriveEditor.document, this.customizableLinksSelector);
        if (isInMedia || (link && link.isContentEditable)) {
            ...
            this.tooltipTimeouts.push(setTimeout(() => {
                ...
                this.thriveEditor.observerUnactive();
                ...
                this.thriveEditor.observerActive();
            }, 400));
        }
    ...
    _updateMediaJustifyButton: function (commandState) {
        ...
        // Change the ID to prevent ThriveEditor from controlling it as this is
        ...
        const commands = [
            {
                ...
                isDisabled: () => !this.thriveEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.thriveEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.thriveEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.thriveEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (res && res[0] && res[0].signature) {
                        this.thriveEditor.execCommand('insert', parseHTML(res[0].signature));
                    }
                },
            },
            {
                ...
                callback: () => this.thriveEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.thriveEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
                    const anchor = this.thriveEditor.document.getSelection().anchorNode;
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.thriveEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.thriveEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.thriveEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.thriveEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.thriveEditor.execCommand('columnize', 0),
                isDisabled: () => {
                    if (!this.thriveEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
    ...
    _saveElement: function ($el, context, withLang) {
        ...
        // remove ZeroWidthSpace from thrive field value
        // ZeroWidthSpace may be present from Thrive Bureau ERPEditor edition process
    ...
    _onSelectionChange() {
        if (this.options.autohideToolbar) {
            ...
            if (isVisible && !this.thriveEditor.document.getSelection().isCollapsed) {
                this.linkPopover.hide();
            }
        }
    },
    ...
    _signalOnline: async function () {
        ...
        try {
            ...
            if (currentContent !== dbContent && dbRecordDate !== currentRecordDate) {
                ...
                $dialogContent.append($(this.thriveEditor.editable).clone());
    ...
    resetEditor: function (value, options) {
        ...
        this._startCollaborationTime = new Date().getTime();
        this.ptp = this._getNewPtp();
        this.thriveEditor.collaborationSetClientId(this._currentClientId);
        this.setValue(value);
        this.thriveEditor.historyReset();
                    ...
                    for (const record of records) {
                        if (record.type === 'attributes' && record.attributeName === 'class') {
                            ...
                            if (!node.classList.contains('o_not_editable')) {
                                this.thriveEditor.observerUnactive('_setONotEditable');
                                node.removeAttribute('contenteditable');
                                this.thriveEditor.observerActive('_setONotEditable');
                                observer.disconnect();
...
thrive.define('web_editor.widget', function (require) {
'use strict';
    return {
