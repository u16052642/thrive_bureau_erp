thrive.define('web_editor.loader', function (require) {
'use strict';
...
async function getWysiwygClass({moduleName = 'web_editor.wysiwyg', additionnalAssets = []} = {}) {
    if (!(await thrive.ready(moduleName))) {
        await loadWysiwyg(additionnalAssets);
        await thrive.ready(moduleName);
    }
    return thrive.__DEBUG__.services[moduleName];
}
