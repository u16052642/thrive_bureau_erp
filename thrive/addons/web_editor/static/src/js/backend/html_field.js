...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/thrive-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.thriveEditor) {
            const selection = this.wysiwyg.thriveEditor.document.getSelection();
    ...
    toggleCodeView() {
        ...
        this.wysiwyg.thriveEditor.observerUnactive('toggleCodeView');
        if (this.state.showCodeView) {
            this.wysiwyg.thriveEditor.toolbarHide();
            const value = this.wysiwyg.getValue();
            this.props.update(value);
        } else {
            this.wysiwyg.thriveEditor.observerActive('toggleCodeView');
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.thriveEditor.execCommand('insert', t);
        }
    }
    ...
    async commitChanges({ urgent } = {}) {
        if (this._isDirty() || urgent) {
            if (this.wysiwyg) {
                ...
                this.wysiwyg.thriveEditor.observerUnactive('commitChanges');
                await this.wysiwyg.saveModifiedImages();
                if (this.props.isInlineStyle) {
                    await this._toInline();
                }
                this.wysiwyg.thriveEditor.observerActive('commitChanges');
            }
    ...
    async _toInline() {
        ...
        const $thriveEditor = $editable.closest('.thrive-editor-editable');
        ...
        $thriveEditor.removeClass('thrive-editor-editable');
        $editable.html(html);

        await toInline($editable, this.cssRules, this.wysiwyg.$iframe);
        $thriveEditor.addClass('thrive-editor-editable');
...
HtmlField.extractProps = ({ attrs, field }) => {
    return {
        ...
        codeview: Boolean(thrive.debug && attrs.options.codeview),
