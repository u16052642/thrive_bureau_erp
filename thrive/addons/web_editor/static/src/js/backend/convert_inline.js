/** @thrive-module alias=web_editor.convertInline */
'use strict';

import { isBlock, rgbToHex } from '../editor/thrive-editor/src/utils/utils';
...
async function toInline($editable, cssRules, $iframe) {
    $editable.removeClass('thrive-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('thrive-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const fonts = thrive.__DEBUG__.services["wysiwyg.fonts"];
