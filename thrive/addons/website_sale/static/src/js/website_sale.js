thrive.define('website_sale.cart', function (require) {
'use strict';
...
thrive.define('website_sale.website_sale_offcanvas', function (require) {
'use strict';
...
thrive.define('website_sale.website_sale', function (require) {
'use strict';
        ...
        if (salePage.dataset.ecomZoomAuto) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomThrive = $image.data("zoomThrive");
                    if (zoomThrive) {
                        zoomThrive.hide();
                        $image.unbind();
                    }
        ...
        if (salePage.dataset.ecomZoomClick) {
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoomthrive-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.thriveEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.thriveEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
...
thrive.define('website_sale.price_range_option', function (require) {
'use strict';
