...
export default class PivotAutofillPlugin extends spreadsheet.UIPlugin {
    ...
    getPivotNextAutofillValue(formula, isColumn, increment) {
        ...
        for (let i = evaluatedArgs.length - 1; i > 0; i--) {
            ...
            if (
                ...
                if (functionName === "THRIVE.PIVOT") {
                    return makePivotFormula("THRIVE.PIVOT", evaluatedArgs);
                } else if (functionName === "THRIVE.PIVOT.HEADER") {
                    return makePivotFormula("THRIVE.PIVOT.HEADER", evaluatedArgs);
                }
                return formula;
            }
        }
    ...
    _autofillPivotValue(pivotId, args, isColumn, increment) {
        ...
        return makePivotFormula("THRIVE.PIVOT", this._buildArgs(pivotId, measure, rows, cols));
    }
    /**
    ...
    _autofillColFromValue(pivotId, nextIndex, currentElement) {
        ...
        return makePivotFormula("THRIVE.PIVOT.HEADER", this._buildArgs(pivotId, undefined, [], cols));
    }
    /**
    ...
    _autofillRowFromValue(pivotId, currentElement) {
        ...
        return makePivotFormula("THRIVE.PIVOT.HEADER", this._buildArgs(pivotId, undefined, rows, []));
    }
    /**
