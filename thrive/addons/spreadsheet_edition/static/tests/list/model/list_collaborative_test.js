...
function insertList(model, id, anchor = [0, 0]) {
    ...
    return model.dispatch("INSERT_THRIVE_LIST", {
...
QUnit.test("Add two lists concurrently", async (assert) => {
    ...
    assert.spreadsheetIsSynchronized(
        ...
        `=THRIVE.LIST.HEADER(1,"foo")`
    );
    assert.spreadsheetIsSynchronized(
        [alice, bob, charlie],
        (user) => getCellFormula(user, "A26"),
        `=THRIVE.LIST.HEADER(2,"foo")`
    );
    await nextTick();
...
QUnit.test("Can undo a command before a INSERT_THRIVE_LIST", async (assert) => {
    assert.expect(1);
...
QUnit.test("Rename and remove a list concurrently", async (assert) => {
    insertList(alice, "1");
    await network.concurrent(() => {
        alice.dispatch("RENAME_THRIVE_LIST", {
            listId: "1",
            name: "test",
        });
        bob.dispatch("REMOVE_THRIVE_LIST", {
            listId: "1",
        });
    });
    assert.spreadsheetIsSynchronized(
...
QUnit.test("Re-insert and remove a list concurrently", async (assert) => {
    ...
    await network.concurrent(() => {
        ...
        alice.dispatch("RE_INSERT_THRIVE_LIST", {
            id: "1",
            col: 0,
            row: 0,
