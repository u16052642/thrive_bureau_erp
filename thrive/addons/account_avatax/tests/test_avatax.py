...
class TestAccountAvalaraInternal(TestAccountAvataxCommon):
    ...
    def test_01_thrive_invoice(self):
        ...
        with patch('thrive.addons.account_avatax.models.account_avatax.AccountAvatax._uncommit_avatax_transaction') as mocked_commit:
...
class TestAccountAvalaraSalesTaxAdministration(TestAccountAvataxCommon):
    ...
    def test_disable_document_recording(self):
        ...
        invoice, response = self._create_invoice_01_and_expected_response()
        with self._capture_request(return_value=response), patch('thrive.addons.account_avatax.lib.avatax_client.AvataxClient.commit_transaction') as mocked_commit:
            invoice.action_post()
