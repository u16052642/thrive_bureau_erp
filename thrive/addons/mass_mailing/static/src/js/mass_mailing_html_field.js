...
import { getRangePosition } from '@web_editor/js/editor/thrive-editor/src/utils/utils';
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges() {
        ...
        this.wysiwyg.thriveEditor.observerUnactive('toInline');
        await toInline($editable, this.cssRules, this.wysiwyg.$iframe);
        this.wysiwyg.thriveEditor.observerActive('toInline');
        const inlineHtml = $editable.html();
        $editorEnable.addClass('editor_enable');
        this.wysiwyg.thriveEditor.resetContent(initialHtml);
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.thrive-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.thriveEditor.observerFlush();
        this.wysiwyg.thriveEditor.historyReset();
        this.wysiwyg.$iframeBody.addClass('o_mass_mailing_iframe');

        this.wysiwyg.thriveEditor.document.addEventListener('mousedown', () => {
    ...
    async _onSnippetsLoaded() {
        ...
        $snippetsSideBar.on('click', '.o_codeview_btn', () => {
            this.wysiwyg.thriveEditor.observerUnactive();
            $codeview.toggleClass('d-none');
            this.wysiwyg.getEditable().toggleClass('d-none');
            this.wysiwyg.thriveEditor.observerActive();
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Thrive editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.thriveEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.thriveEditor.editable.querySelector('p');
                if (p) {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.thriveEditor.resetContent($newLayout[0].outerHTML);
        ...
        this.onIframeUpdated();
        this.wysiwyg.thriveEditor.historyStep(true);
