...
from thrive import _, api, exceptions, fields, models, tools, registry, SUPERUSER_ID, Command
from thrive.exceptions import MissingError, AccessError
from thrive.osv import expression
from thrive.tools import is_html_empty
from thrive.tools.misc import clean_context, split_every
...
class MailThread(models.AbstractModel):
    ...
    def _detect_loop_sender(self, message, message_dict, routes):
        ...
        send emails to Thrive Bureau ERP.
        """
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if thread._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # parsing should find an author independently of user running mail gateway, and ensure it is not thrivebot
    ...
    def message_post(self, *,
        ...
        if 'channel_ids' in kwargs:
            raise ValueError(_("Posting a message with channels as listeners is not supported since Thrive 14.3+. Please update code accordingly."))
        ...
        if msg_values['author_id'] and msg_values['message_type'] != 'notification' and not self._context.get('mail_create_nosubscribe'):
            if self.env['res.partner'].browse(msg_values['author_id']).active:  # we dont want to add thrivebot/inactive as a follower
    ...
    def _notify_by_email_get_base_mail_values(self, message, additional_values=None):
        ...
        # SES SMTP may replace Message-Id and In-Reply-To refers an internal ID not stored in Thrive)
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        with track_visibility set. Since ThriveERP v7 it is considered as being
