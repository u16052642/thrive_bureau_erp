...
registerModel({
    ...
    recordMethods: {
        ...
        async _handleNotificationChannelMessage({ id: channelId, message: messageData }) {
            ...
            // Chat from Thrive Bureau ERPBot is considered disturbing and should only be
            // shown on the menu, but no notification and no thread open.
            const isChatWithThriveBot = (
                channel.correspondent &&
                channel.correspondent === this.messaging.partnerRoot
            );
            if (!isChatWithThriveBot) {
                const isThriveFocused = this.env.services['presence'].isThriveFocused();
                // Notify if out of focus
                if (!isThriveFocused && channel.thread.isChatChannel) {
