...
QUnit.test('receive new chat message: out of thrive focus (notification, channel)', async function (assert) {
    assert.expect(4);
    ...
    const { env, openDiscuss } = await start({
        services: {
            presence: makeFakePresenceService({ isThriveFocused: () => false }),
        },
    });
    await openDiscuss();
    ...
    // simulate receiving a new message with thrive focused
    await afterNextRender(() => {
...
QUnit.test('receive new chat message: out of thrive focus (notification, chat)', async function (assert) {
    assert.expect(4);
...
QUnit.test('receive new chat messages: out of thrive focus (tab title)', async function (assert) {
    assert.expect(12);
    ...
    const mailChannel1 = pyEnv['mail.channel'].searchRead([['id', '=', mailChannelId1]])[0];
    // simulate receiving a new message in chat 1 with thrive focused
    await afterNextRender(() => {
        pyEnv['bus.bus']._sendone(mailChannel1, 'mail.channel/new_message', {
            'id': mailChannelId1,
            'message': {
    ...
    // simulate receiving a new message in chat 2 with thrive focused
    await afterNextRender(() => {
    ...
    // simulate receiving another new message in chat 2 with thrive focused
    await afterNextRender(() => {
        pyEnv['bus.bus']._sendone(mailChannel2, 'mail.channel/new_message', {
            'id': mailChannelId2,
            'message': {
