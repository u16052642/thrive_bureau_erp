...
from thrive.addons.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Thrive event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Thrive events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Thrive Bureau ERP events
        # -----------------------------------------------------------------------------------------

        self.expected_thrive_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_thrive_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_thrive_recurrency_events_from_outlook = [
            {
    ...
    def assert_thrive_event(self, thrive_event, expected_values):
        """
        Assert that an Thrive event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        thrive_event_values = thrive_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(thrive_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, thrive_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_thrive_recurrence(self, thrive_recurrence, expected_values):
        """
        Assert that an Thrive recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        thrive_recurrence_values = thrive_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, thrive_recurrence_values.get(k), msg=f"'{k}' mismatch")
