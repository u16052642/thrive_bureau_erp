...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by Thrive recordset, one instance can be a single Microsoft event or a
    ...
    def thrive_id(self, env):
        return self._thrive_id

    def _meta_thrive_id(self, microsoft_guid):
        """Returns the Thrive id stored in the Microsoft Event metadata.
    ...
    def thrive_ids(self):
        """
        Get the list of Thrive event ids already mapped with Outlook events (self)
        """
        return tuple(e._thrive_id for e in self if e._thrive_id)

    def _load_thrive_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing Thrive events:
        1) extract unmapped events only,
        2) match Thrive events and Outlook events which have both a ICalUId set,
        3) match remaining events,
        Returns the list of mapped events
        """
        mapped_events = [e.id for e in self if e._thrive_id]
        ...
        thrive_events = model_env.with_context(active_test=False).search([
            '|',
            ('ms_universal_event_id', "in", unmapped_events.uids),
            ('ms_organizer_event_id', "in", unmapped_events.ids)
        ]).with_env(env)

        # 1. try to match unmapped events with Thrive events using their iCalUId
        unmapped_events_with_uids = unmapped_events.filter(lambda e: e.iCalUId)
        thrive_events_with_uids = thrive_events.filtered(lambda e: e.ms_universal_event_id)
        mapping = {e.ms_universal_event_id: e.id for e in thrive_events_with_uids}

        for ms_event in unmapped_events_with_uids:
            thrive_id = mapping.get(ms_event.iCalUId)
            if thrive_id:
                ms_event._events[ms_event.id]['_thrive_id'] = thrive_id
                mapped_events.append(ms_event.id)

        # 2. try to match unmapped events with Thrive events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        mapping = {e.ms_organizer_event_id: e for e in thrive_events}

        for ms_event in unmapped_events:
            thrive_event = mapping.get(ms_event.id)
            if thrive_event:
                ms_event._events[ms_event.id]['_thrive_id'] = thrive_event.id
                mapped_events.append(ms_event.id)

                # don't forget to also set the global event ID on the Thrive event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current Thrive user is the organizer of the event according to Outlook event, so return his id.
        2) the current Thrive user is NOT the organizer and:
           2.1) we are able to find a Thrive user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a Thrive user matching the organizer email address and we return False, meaning
                that no Thrive user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        if self.organizer.get('emailAddress') and self.organizer.get('emailAddress').get('address'):
            # Warning: In Microsoft: 1 email = 1 user; but in Thrive several users might have the same email
    ...
    def match_with_thrive_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing Thrive events, and return the list of matched events
        """
        ...
        mapped_recurrences = recurrence_candidates._load_thrive_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_thrive_ids_from_db(env)
