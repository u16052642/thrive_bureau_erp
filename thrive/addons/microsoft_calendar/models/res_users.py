...
import logging
import requests
from thrive.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
from datetime import timedelta

from thrive import api, fields, models, _
from thrive.exceptions import UserError
from thrive.loglevels import exception_to_unicode
from thrive.addons.microsoft_account.models.microsoft_service import DEFAULT_MICROSOFT_TOKEN_ENDPOINT
from thrive.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Thrive
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2thrive(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Thrive -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_thrive2microsoft()
        ...
        (events - synced_events)._sync_thrive2microsoft()
