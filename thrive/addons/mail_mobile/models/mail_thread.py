...
from thrive import models, api, tools
from thrive.addons.iap.tools import iap_tools

_logger = logger.getLogger(__name__)

MOBILE_APP_IDENTIFIER = 'com.thrive.mobile'
...
class MailThread(models.AbstractModel):
    ...
    def _notify_thread_by_ocn(self, message, recipients_data, msg_vals=False, **kwargs):
        ...
        if not icp_sudo.get_param('thrive_ocn.project_id') or not icp_sudo.get_param('mail_mobile.enable_ocn'):
            return
        ...
        # never send to author and to people outside of thrive (email), except comments
        if msg_type == 'comment':
