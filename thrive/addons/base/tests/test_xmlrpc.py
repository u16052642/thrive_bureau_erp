...
from thrive.exceptions import AccessDenied, AccessError
from thrive.http import _request_stack

import thrive.tools
from thrive.tests import common
from thrive.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, thrive.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
