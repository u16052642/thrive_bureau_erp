...
import thrive
from thrive.http import Controller, route, dispatch_rpc, request
from thrive.fields import Date, Datetime, Command
from thrive.tools import lazy, ustr
...
# User code must use the exceptions defined in ``thrive.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, thrive.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, thrive.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, thrive.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, thrive.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, thrive.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(thrive.tools.exception_to_unicode(e), formatted_info)

    return xmlrpc.client.dumps(fault, allow_none=None, encoding=None)


class ThriveMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, thrive is sending binary as base64 string.
...
xmlrpc.client.Marshaller = ThriveMarshaller
...
class RPC(Controller):
    ...
    def jsonrpc(self, service, method, args):
        """ Method used by client APIs to contact ThriveERP. """
