...
from thrive import api, fields, models, tools, SUPERUSER_ID, _, Command
from thrive.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from thrive.exceptions import AccessDenied, AccessError, UserError, ValidationError
from thrive.http import request, DEFAULT_LANG
from thrive.osv import expression
from thrive.service.db import check_super
from thrive.tools import is_html_empty, partition, collections, frozendict, lazy_property
...
class Users(models.Model):
    """ User class. A res.users record models an ThriveERP user and is different
        from an employee.
    ...
    def _unlink_except_superuser(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Thrive (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: thrive.exceptions.AccessDenied when old password is wrong
        :raise: thrive.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Thrive portal anymore.
