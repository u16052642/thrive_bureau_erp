...
import thrive
from thrive import api, fields, models, tools, _, Command
from thrive.exceptions import MissingError, ValidationError, AccessError
from thrive.tools import frozendict
from thrive.tools.safe_eval import safe_eval, test_python_expr
from thrive.tools.float_utils import float_compare
...
class IrActionsServer(models.Model):
    ...
    Since Thrive 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: Thrive Environment on which the action is triggered
#  - model: Thrive Model of the record on which the action is triggered; is a void recordset
#  - record: record on which the action is triggered; may be void
#  - records: recordset of all records on which the action is triggered in multi-mode; may be void
#  - time, datetime, dateutil, timezone: useful Python libraries
#  - float_compare: Thrive function to compare floats based on specific precisions
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': thrive.exceptions.Warning,
            'UserError': thrive.exceptions.UserError,
            # record
            'record': record,
            'records': records,
            # helpers
