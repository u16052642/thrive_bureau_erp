...
BASE_VERSION = thrive.modules.get_manifest('base')['version']
...
class ir_cron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('THRIVE_NOTIFY_CRON_CHANGES'):
        ...
        try:
            ...
            with db.cursor() as cron_cr:
                ...
                for job_id in (job['id'] for job in jobs):
                    ...
                    registry = thrive.registry(db_name)
            ...
            if e.pgcode == '42P01':
                ...
                # The table ir_cron does not exist; this is probably not an ThriveERP database.
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        thrive.modules.reset_modules_state(cr.dbname)
    ...
    def _callback(self, cron_name, server_action_id, job_id):
        ...
        try:
            ...
            thrive.netsvc.log(_logger, logging.DEBUG, 'cron.object.execute', (self._cr.dbname, self._uid, '*', cron_name, server_action_id), depth=log_depth)
            start_time = False
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('THRIVE_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('THRIVE_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)

    def _notifydb(self):
        """ Wake up the cron workers
        The THRIVE_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with thrive.sql_db.db_connect('postgres').cursor() as cr:
