...
from thrive import api, fields, models, tools, SUPERUSER_ID, _
from thrive.exceptions import UserError, AccessError
from thrive.tools.safe_eval import safe_eval, time
from thrive.tools.misc import find_in_path
from thrive.tools import check_barcode_encoding, config, is_html_empty, parse_version
from thrive.http import request
from thrive.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of Thrive so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Thrive start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Thrive with at least two workers to print a pdf version of the reports.')
    ...
    def _merge_pdfs(self, streams):
        ...
        for stream in streams:
            try:
                ...
                raise UserError(_("Thrive is unable to merge the generated PDFs."))
