...
import lxml.html
import thrive
from thrive.addons.http_routing.models.ir_http import url_lang
...
class TestLangUrl(HttpCase):
    ...
    def test_04_url_cook_lang_not_available(self):
        ...
        for line in r.text.splitlines():
            _, match, session_info_str = line.partition('thrive.__session_info__ = ')
            if match:
    ...
    def test_01_controller_redirect(self):
        ...
        def assertUrlRedirect(url, expected_url, msg="", code=301):
            if expected_url.startswith('/'):
                expected_url = "http://%s:%s%s" % (HOST, thrive.tools.config['http_port'], expected_url)
            if not msg:
