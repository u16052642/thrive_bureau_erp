...
import thrive.tests
from thrive.addons.iap.tools.iap_tools import iap_jsonrpc_mocked
from thrive.tools import mute_logger

class TestConfiguratorCommon(thrive.tests.HttpCase):
    ...
    def setUp(self):
        ...
        iap_patch = patch('thrive.addons.iap.tools.iap_tools.iap_jsonrpc', iap_jsonrpc_mocked_configurator)
        self.startPatcher(iap_patch)

        patcher = patch('thrive.addons.website.models.ir_module_module.IrModuleModule._theme_upgrade_upstream', wraps=self._theme_upgrade_upstream)
