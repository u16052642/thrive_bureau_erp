...
import thrive
from thrive.tests.common import HttpCase, HOST
from thrive.tools.misc import DotDict, frozendict
...
def MockRequest(
    ...
    request = Mock(
        ...
        type='http',
        future_response=thrive.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            thrive.http.get_default_session(),
    ...
    #
    #   router = thrive.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        thrive.http._request_stack.push(request)
        s.callback(thrive.http._request_stack.pop)
        s.enter_context(patch('thrive.http.root.get_db_router', router))
