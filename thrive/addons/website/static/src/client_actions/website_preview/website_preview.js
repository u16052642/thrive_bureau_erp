...
export class WebsitePreview extends Component {
    setup() {
        ...
        onMounted(() => {
            ...
            // ThriveFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('ThriveFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        ...
        /**
         * This removes the 'Thrive' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('ThriveFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('ThriveFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.iframe.el.addEventListener('ThriveFrameContentLoaded', resolve, { once: true });
            if (url) {
