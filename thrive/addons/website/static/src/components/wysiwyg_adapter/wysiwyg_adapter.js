...
export class WysiwygAdapterComponent extends ComponentAdapter {
    ...
    setup() {
        ...
        useEffect(() => {
            const initWysiwyg = async () => {
                this.$editable.on('click.thrive-website-editor', '*', this, this._preventDefault);
                // Disable ThriveEditor observer's while setting up classes
                this.widget.thriveEditor.observerUnactive();
                ...
                this.widget.thriveEditor.observerActive();
            };

            initWysiwyg();

            return () => {
                this.$editable.off('click.thrive-website-editor', '*');
            };
        }, () => []);
    ...
    leaveEditMode({ onLeave, forceLeave, onStay, reloadIframe = true } = {}) {
        const leave = () => {
            ...
            // The wysiwyg is destroyed to avoid listeners from the ThriveEditor
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.widget.thriveEditor.filterMutationRecords(records);
            ...
            this.widget.thriveEditor.automaticStepSkipStack();
        ...
        this.widget.thriveEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.widget.thriveEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
        ...
        const commands = [
            {
                ...
                fontawesome: 'fa-info',
                isDisabled: () => !this.widget.thriveEditor.isSelectionInBlockRoot(),
                callback: () => {
