thrive.define('l10n_de_pos_cert.pos', function(require) {
    "use strict";
    ...
    const L10nDePosGlobalState = (PosGlobalState) => class L10nDePosGlobalState extends PosGlobalState {
         ...
         * send it to Thrive Bureau ERP. Two cases can happen:
         * - Failure to send to Fiskaly => we assume that if one order fails, EVERY order will fail
         * - Failure to send to Thrive Bureau ERP => the order is already sent to Fiskaly, we store them locally with the TSS info
         */
        async _flush_orders(orders, options) {
            ...
            if (result && fiskalyFailure.length === 0) {
                ...
                throw thriveError || fiskalyError;
            }
        }
    }
