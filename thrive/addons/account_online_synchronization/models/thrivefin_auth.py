...
class ThriveFinAuth(requests.auth.AuthBase):
    """ This is used to sign the request going towards ThriveFin
        e.g.:
            requests.get(THRIVEFIN + '/example', auth=ThriveFinAuth())
            By using `auth=ThriveFinAuth(self)` when doing a http request, the request will be signed and
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'thrivefin-client-id': self.client_id,
            'thrivefin-access-token': self.access_token,
            'thrivefin-signature': base64.b64encode(h.digest()),
            'thrivefin-timestamp': msg_timestamp,
        })
        return request
