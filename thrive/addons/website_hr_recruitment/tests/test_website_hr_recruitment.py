...
from thrive.api import Environment
import thrive.tests
from thrive.tools import html2plaintext

@thrive.tests.tagged('post_install', '-at_install')
class TestWebsiteHrRecruitmentForm(thrive.tests.HttpCase):
    def test_tour(self):
        ...
        with thrive.tests.RecordCapturer(self.env['hr.applicant'], []) as capt:
