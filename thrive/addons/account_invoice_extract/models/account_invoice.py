...
from thrive import api, fields, models, _, _lt, Command
from thrive.addons.iap.tools import iap_tools
from thrive.exceptions import AccessError, UserError
from thrive.tools import float_compare, mute_logger
...
ERROR_MESSAGES = {
    ...
    ERROR_INVALID_ACCOUNT_TOKEN: _lt("The 'invoice_ocr' IAP account token is invalid. Please delete it to let Thrive generate a new one or fill it with a valid token."),
    ...
    def _save_form(self, ocr_results, force_write=False):
        ...
        with self._get_edi_creation() as move_form:
            ...
            if self.is_sale_document():
                with mute_logger('thrive.tests.common.onchange'):
