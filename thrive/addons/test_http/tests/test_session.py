...
import thrive
from thrive.tests.common import get_db_name
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('thrive.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session1_default_session(self):
        ...
        with patch.object(thrive.http.root.session_store, 'save') as mock_save:
    ...
    def test_session2_geoip(self):
        real_save = thrive.http.root.session_store.save
        with patch.object(thrive.http.root.geoip_resolver, 'resolve') as mock_resolve,\
             patch.object(thrive.http.root.session_store, 'save') as mock_save:
            mock_resolve.return_value = GEOIP_THRIVE_FARM_2
            ...
            res.raise_for_status()
            self.assertEqual(res.text, str(GEOIP_THRIVE_FARM_2))
            mock_save.assert_not_called()
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        with self.assertLogs('thrive.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:thrive.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
