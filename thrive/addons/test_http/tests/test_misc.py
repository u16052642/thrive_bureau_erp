...
import thrive
from thrive.http import root
from thrive.tests import tagged
from thrive.tests.common import HOST, new_test_user, get_db_name
from thrive.tools import config, file_path
from thrive.addons.test_http.controllers import CT_JSON
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = thrive.tools.misc.DotDict(db=False)
            return thrive.http.Request.redirect(fake_req, path, local=True).headers['Location']
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(thrive.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = thrive.http.root.session_store.get(res.cookies['session_id'])
