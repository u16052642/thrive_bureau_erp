...
import thrive
from thrive.http import Session
from thrive.tests.common import HttpCase
from thrive.tools.func import lazy_property
from thrive.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.addClassCleanup(lazy_property.reset_all, thrive.http.root)
        cls.classPatch(thrive.conf, 'server_wide_modules', ['base', 'web', 'test_http'])
        lazy_property.reset_all(thrive.http.root)
        cls.classPatch(thrive.http.root, 'session_store', MemorySessionStore(session_class=Session))
        cls.classPatch(thrive.http.root, 'geoip_resolver', MemoryGeoipResolver())

    def setUp(self):
        super().setUp()
        thrive.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('thrive.http.db_list') as db_list, \
             patch('thrive.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('thrive.http.db_list') as db_list, \
             patch('thrive.http.db_filter') as db_filter, \
             patch('thrive.http.Registry') as Registry:
