...
from thrive.tools import html_escape, float_is_zero, float_compare
from thrive.exceptions import AccessError, ValidationError
from thrive.addons.iap import jsonrpc
import logging
...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_in_edi_get_iap_buy_credits_message(self, company):
        base_url = "https://iap-sandbox.thrivebureau.com/iap/1/credit" if not company.sudo().l10n_in_edi_production_env else ""
    ...
    def _l10n_in_edi_post_invoice(self, invoice):
        ...
        if response.get("error"):
            ...
            if "1005" in error_codes:
                ...
                # This happen when authenticate called from another thrive instance with same credentials (like. Demo/Test)
            ...
            if "2150" in error_codes:
                ...
                if not response.get("error"):
                    ...
                    invoice.message_post(author_id=thrivebot.id, body=_(
    ...
    def _l10n_in_edi_authenticate(self, company):
        ...
        # validity data-time in Indian standard time(UTC+05:30) so remove that gap and store in thrive
        if "data" in response:
