...
class SocialStreamFacebook(models.Model):
    ...
    def _format_facebook_message(self, message, tags):
        ...
        example:
            "Hello Thrive :)" -> "Hello @[542132] Thrive :)"
            "Hello Thrive Social :)" -> "Hello @[542132] Thrive-Social :)"
            "Hello Thrive - Social :)" -> "Hello @[542132] Thrive-Social :)"

        details:
            "Hello @[42] faketag Thrive - Social :)" -> "Hello @ [42] faketag @[542132] Thrive-Social :)""
            -> With associated tag: {name: "Thrive - Social", offset: 20, length: 13}

            We take the message until the tag offset: "Hello @[42] faketag "
            and remove the fake tag in the process "Hello @ [42] faketag "
            Then we insert the forged tag "@[542132] Thrive-Social"
