...
ThriveERP domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    an ThriveERP domain expression from an untrusted source.
    ...
    except thrive.exceptions.UserError:
        raise
    except thrive.exceptions.RedirectWarning:
        raise
...
`thrive.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `thrive.tools.safe_eval`.
""")
    return d
