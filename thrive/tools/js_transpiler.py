"""
This code is what let us use ES6-style modules in thrive.
Classic Thrive modules are composed of a top-level :samp:`thrive.define({name},{body_function})` call.
This processor will take files starting with an `@thrive-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** thrive-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual thrive one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom thrive modules.
    ...
    legacy_thrive_define = get_aliased_thrive_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_thrive_define, module_path),
    ]
    for s in steps:
        content = s(content)
    if legacy_thrive_define:
        content += legacy_thrive_define
    return content
...
def url_to_module_path(url):
    """
    Thrive modules each have a name. (thrive.define("<the name>", async function (require) {...});
...
def wrap_with_thrive_define(module_path, content):
    """
    Wraps the current content (source code) with the thrive.define call.
    Should logically be called once all other operations have been performed.
    """
    return f"""thrive.define({module_path!r}, async function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
THRIVE_MODULE_RE = re.compile(r"""
    \s*                                       # some starting space
    \/(\*|\/).*\s*                            # // or /*
    @thrive-module                              # @thrive-module
...
def is_thrive_module(content):
    """
    Detect if the file is a native thrive module.
    We look for a comment containing @thrive-module.

    :param content: source code
    :return: is this a thrive module that need transpilation ?
    """
    result = THRIVE_MODULE_RE.match(content)
    return bool(result)


def get_aliased_thrive_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the thrive source base being:
    ...
    > thrive.define("web/chrome/abstract_action", async function(require) {
    ...
    .. code-block:: javascript
        ...
        /** @thrive-module */
        // after
        /** @thrive-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @thrive-module */
        // after
        /** @thrive-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = THRIVE_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nthrive.define(`%s`, async function(require) {
                        return require('%s');
                        });\n""" % (alias, module_path)
            else:
                return """\nthrive.define(`%s`, async function(require) {
