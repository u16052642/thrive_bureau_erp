...
"""
Miscellaneous tools used by ThriveERP.
"""
import cProfile
import collections
import contextlib
...
import thrive
import thrive.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from thrive.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if thrive.tools.config['db_host']:
        env['PGHOST'] = thrive.tools.config['db_host']
    if thrive.tools.config['db_port']:
        env['PGPORT'] = str(thrive.tools.config['db_port'])
    if thrive.tools.config['db_user']:
        env['PGUSER'] = thrive.tools.config['db_user']
    if thrive.tools.config['db_password']:
        env['PGPASSWORD'] = thrive.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',)):
    ...
    addons_paths = thrive.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/thrive/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def scan_languages():
    """ Returns all languages supported by ThriveERP for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = thrive.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('thrive.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('thrive.foo.bar'):
            do_suff()
    """
...
def ignore(*exc):
    warnings.warn("Since 16.0 `thrive.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = thrive.fields.Datetime.from_string(value)
            value = thrive.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = thrive.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = thrive.fields.Datetime.from_string(value)
    else:
        timestamp = value
