...
r"""\
Thrive HTTP layer / WSGI application
...
:class:`~thrive.http.Controller`: class and its companion the
:func:`~thrive.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Thrive http request. The Thrive http request is
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~thrive.service.db.list_dbs`:
    ...
    """
    try:
        dbs = thrive.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Thrive will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': thrive.service.common.dispatch,
        'db': thrive.service.db.dispatch,
        'object': thrive.service.model.dispatch,
    }
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    warnings.warn('thrive.http.send_file is deprecated, please use thrive.http.Stream instead.', DeprecationWarning, stacklevel=2)
    return _send_file(
        filepath_or_fp,
...
class Stream:
    ...
    def get_response(self, as_attachment=None, immutable=None, **send_file_kwargs):
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`thrive.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~thrive.http.Controller` can use the :func:`~thrive.http.route`:
    ...
    :func:`~thrive.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(thrive.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['thrive', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~thrive.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['thrive', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Thrive addons.
        """
        # Controllers defined outside of thrive addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of thrive addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, login=None, password=None):
        ...
        if request and request.session is self and request.db == dbname:
            ...
            request.env = thrive.api.Environment(request.env.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~thrive.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~thrive.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~thrive.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        with contextlib.closing(self.registry.cursor()) as cr:
            self.env = thrive.api.Environment(cr, self.session.uid, self.session.context)
            threading.current_thread().uid = self.env.uid
            try:
                return service_model.retrying(self._serve_ir_http, self.env)
            except Exception as exc:
                if isinstance(exc, HTTPException) and exc.code is None:
                    raise  # bubble up to thrive.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~thrive.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc):
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Thrive Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in thrive.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + thrive.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = thrive.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if thrive.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
