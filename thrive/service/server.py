...
import thrive
from thrive.modules import get_modules
from thrive.modules.registry import Registry
from thrive.release import nt_service_name
from thrive.tools import config
from thrive.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'thrive.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The THRIVE_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("THRIVE_MAX_HTTP_THREADS")
    ...
    # is included in all Python 3 releases installed on all operating systems supported by Thrive.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(thrive, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in thrive.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="thrive.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from thrive.addons.base.models.ir_cron import ir_cron
        conn = thrive.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = thrive.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(thrive.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="thrive.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="thrive.service.httpd")
        t.daemon = True
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(thrive, 'phoenix', None):
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = thrive.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('thrive: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(thrive.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The THRIVE_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("THRIVE_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = thrive.service.db.list_dbs(True)
        return db_names
    ...
    def process_work(self):
        ...
        if len(db_names):
            ...
            from thrive.addons import base
    ...
    def start(self):
        ...
        dbconn = thrive.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(thrive.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            thrive.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if thrive.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from thrive.tests.common import ThriveSuite
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = thrive.sql_db.sql_counter
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = thrive.api.Environment(cr, thrive.SUPERUSER_ID, {})
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             thrive.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the thrive http server and cron processor.
    """
    global server
    ...
    if thrive.evented:
        server = GeventServer(thrive.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(thrive.http.root)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Thrive is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(thrive.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not thrive.evented:
        if inotify:
    ...
    if getattr(thrive, 'phoenix', False):
        _reexec()
