...
import thrive
from thrive import SUPERUSER_ID
from thrive.exceptions import AccessDenied
import thrive.release
import thrive.sql_db
import thrive.tools
from thrive.sql_db import db_connect
from thrive.release import version_info
from thrive.tools import find_pg_tool, exec_pg_environ
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not thrive.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and thrive.tools.config.verify_admin_password(passwd):
        return True
    raise thrive.exceptions.AccessDenied()

# This should be moved to thrive.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = thrive.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            thrive.modules.db.initialize(cr)
            thrive.tools.config['load_language'] = lang
            cr.commit()

        registry = thrive.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = thrive.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = thrive.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = thrive.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = thrive.tools.config['db_template']
    ...
    try:
        db = thrive.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if thrive.tools.config['unaccent']:
                ...
                # But in the case of Thrive, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    thrive.sql_db.close_db(db_original_name)
    db = thrive.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = thrive.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = thrive.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = thrive.tools.config.filestore(db_original_name)
    to_fs = thrive.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    thrive.modules.registry.Registry.delete(db_name)
    thrive.sql_db.close_db(db_name)
    ...
    fs = thrive.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'thrive_dump': '1',
        'db_name': cr.dbname,
        'version': thrive.release.version,
        'version_info': thrive.release.version_info,
        'major_version': thrive.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = thrive.tools.config.filestore(db_name)
            ...
            if stream:
                thrive.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                thrive.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = thrive.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = thrive.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    thrive.modules.registry.Registry.delete(old_name)
    thrive.sql_db.close_db(old_name)
    ...
    old_fs = thrive.tools.config.filestore(old_name)
    new_fs = thrive.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    thrive.tools.config.set_admin_password(new_password)
    thrive.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        thrive.tools.config['update']['base'] = True
        thrive.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@thrive.tools.mute_logger('thrive.sql_db')
...
def list_dbs(force=False):
    if not thrive.tools.config['list_db'] and not force:
        raise thrive.exceptions.AccessDenied()

    if not thrive.tools.config['dbfilter'] and thrive.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Thrive will not
        ...
        res = sorted(db.strip() for db in thrive.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [thrive.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Thrive
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if thrive.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        thrive.sql_db.close_db(database_name)
...
def exp_list_lang():
    return thrive.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(thrive.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
