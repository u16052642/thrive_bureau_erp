    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('thrive.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'thrive.addons'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Thrive models.

    Thrive models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~thrive.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~thrive.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~thrive.models.AbstractModel`.
    """
    ...
    _module = None              #: the model's module (in the Thrive sense)
    ...
    .. warning::
      ...
      :attr:`~thrive.models.Model._inherits`-ed models, the inherited field will
    ...
    def _compute_concurrency_field(self):
        ...
        if self._log_access:
            ...
            self[fname] = thrive.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], thrive.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], thrive.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from thrive.fields import Char, Integer
    ...
    def _read_group_fill_temporal(self, data, groupby, aggregated_fields, annotated_groupbys,
        ...
        if fill_from:
            fill_from = date_utils.start_of(thrive.fields.Datetime.to_datetime(fill_from), granularity)
            if tz:
                fill_from = tz.localize(fill_from)
        elif existing_from:
            fill_from = existing_from
        if fill_to:
            fill_to = date_utils.start_of(thrive.fields.Datetime.to_datetime(fill_to), granularity)
            if tz:
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~thrive.addons.base.models.res_company`).

        For :class:`~thrive.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~thrive.fields.Integer`,
          :class:`~thrive.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~thrive.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~thrive.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~thrive.fields.Many2one`, the value should be the
          database identifier of the record to set
        * The expected value of a :class:`~thrive.fields.One2many` or
          :class:`~thrive.fields.Many2many` relational field is a list of
          :class:`~thrive.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~thrive.fields.Command.create`,
          :meth:`~thrive.fields.Command.update`,
          :meth:`~thrive.fields.Command.delete`,
          :meth:`~thrive.fields.Command.unlink`,
          :meth:`~thrive.fields.Command.link`,
          :meth:`~thrive.fields.Command.clear`, and
          :meth:`~thrive.fields.Command.set`.
        * For :class:`~thrive.fields.Date` and `~thrive.fields.Datetime`,
          ...
          .. warning::
            ...
            :const:`thrive.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
            :const:`thrive.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an ThriveERP domain.
    ...
    def ensure_one(self):
        ...
        :raise thrive.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~thrive.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~thrive.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=lambda item: item[0].write_sequence):
            ...
            if field.relational:
                ...
                for invf in self.pool.field_inverses[field]:
                    ...
                    # /home/dle/src/thrive/master-nochange-fp/thrive/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`thrive.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Thrive models.

    Thrive models are created by inheriting from this class::
