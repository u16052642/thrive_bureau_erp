...
""" ThriveERP core library."""


#----------------------------------------------------------
# thrive must be a namespace package for thrive.addons to become one too
...
import sys
assert sys.version_info > (3, 7), "Outdated python version detected, Thrive requires Python >= 3.7 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in thrive/debian/copyright)
        while 1:
            state = conn.poll()
...
# libc UTC hack
#----------------------------------------------------------
# Make sure the ThriveERP server runs in UTC.
import os
...
from . import fields
from . import api
from thrive.tools.translate import _, _lt
